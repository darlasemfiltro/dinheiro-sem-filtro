...
function normalizeCatIdServer(id: string): string {
  return String(id || '').replace(/_[^_@]+@[^.]+.*$/, '');
}

function isSameCategoryServer(aId: string, bId: string): boolean {
  if (!aId || !bId) return false;
  if (aId === bId) return true;
  return normalizeCatIdServer(aId) === normalizeCatIdServer(bId);
}

function mergeSubcategoryTreesServer(existingSubs: any[] = [], incomingSubs: any[] = [], deletedSubIds: Set<string> = new Set()): any[] {
  const map = new Map<string, any>();
  const normalizeSubId = (id: string) => String(id || '').replace(/_[^_@]+@[^.]+.*$/, '');

  (existingSubs || []).forEach(sub => {
    if (!sub || !sub.id || deletedSubIds.has(sub.id)) return;
    map.set(normalizeSubId(sub.id), { ...sub });
  });

  (incomingSubs || []).forEach(sub => {
    if (!sub || !sub.id || deletedSubIds.has(sub.id)) return;
    const key = normalizeSubId(sub.id);
    const prev = map.get(key);
    if (!prev) {
      map.set(key, { ...sub });
    } else {
      const mergedChildren = mergeSubcategoryTreesServer(prev.subcategories || [], sub.subcategories || [], deletedSubIds);
      map.set(key, {
        ...prev,
        ...sub,
        name: sub.name || prev.name,
        subcategories: mergedChildren,
      });
    }
  });
...
function deduplicateServerData() {
  try {
    ...
    // Ensure Darla accounts have lifetime VIP access if they exist
...
function loadServerFinancials(): Record<string, {
  ...
  const legacyKeys = ['user_carvalho_darlla_gmail_com', 'user_carvalhodarlla_gmail_com', 'darlla-5921'];
  const targetKey = 'carvalho.darlla@gmail.com';
  ...
  const legacyDarlaKeys = ['user_darlasemfiltro_gmail_com', 'user_darla_semfiltro_gmail_com', 'darlla-8704'];
  const targetDarlaKey = 'darla.semfiltro@gmail.com';
...
async function startServer() {
  ...
  app.post('/api/send-reset-email', async (req, res) => {
    try {
      ...
      // Fallback/Parallel: Try Appwrite Cloud Recovery API over HTTPS (port 443)
      try {
        const appwriteRes = await fetch('https://sfo.cloud.appwrite.io/v1/users/recovery', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'X-Appwrite-Project': '6a83a2d30034f2dd2811',
            'X-Appwrite-Key': 'standard_',
          },
          body: JSON.stringify({
            email: cleanEmail,
            url: `https://${req.headers.host || 'dinheirosemfiltro.pages.dev'}/?recovery=true`,
          }),
        });
        if (appwriteRes.ok) {
          emailSent = true;
          console.log('[Appwrite Cloud Recovery] Email dispatched successfully for', cleanEmail);
        }
      } catch (appwriteErr) {
        console.warn('[Appwrite Cloud Recovery Notice]', appwriteErr);
      }
  ...
  app.post('/api/users/register', (req, res) => {
    try {
      ...
      const userRecord: ServerUser = {
        ...
        sexo: String(sexo || gender || 'Outros').trim(),
        consent_lgpd: true,
  ...
  app.post('/api/users/delete', (req, res) => {
    try {
      const { userId, email, requestingUserId } = req.body || {};
      if (!userId && !email) {
      ...
      if (requestingUserId) {
        const canonicalReq = getCanonicalUserIdServer(requestingUserId);
        const isOwner = canonicalReq === canonicalId || requestingUserId === userId || (cleanEmail && requestingUserId.toLowerCase() === cleanEmail);
        if (!isOwner) {
          console.warn('[API Users Delete] Unauthorized attempt by non-owner:', { requestingUserId, targetId: canonicalId });
          return res.status(403).json({ success: false, message: 'Ação bloqueada: Apenas o titular da conta pode excluí-la.' });
        }
      }
      ...
      if (cleanEmail) {
        ...
        Object.keys(financials).forEach((k) => {
          const f = financials[k] as any;
          if (f && (String(f.userId || '').trim().toLowerCase() === cleanEmail || k.toLowerCase().includes(cleanEmail) || k.toLowerCase() === cleanEmail)) {
            delete financials[k];
          }
        });
      }
      ...
      Object.keys(financials).forEach((key) => {
        ...
        if (fin) {
          if (fin.userId && (fin.userId.toLowerCase() === cleanEmail || fin.userId === canonicalId || fin.userId === userId)) {
            delete financials[key];
            return;
          }
          let modified = false;
  ...
  app.get('/api/data/load', (req, res) => {
    try {
      ...
      const deletedSet = new Set<string>(safeEntry.deletedIds || []);
      const filterDeleted = (list: any[]) => (Array.isArray(list) ? list.filter((item: any) => item && item.id && !deletedSet.has(item.id)) : []);
      const fGoals = filterDeleted(safeEntry.financialGoals || safeEntry.goals || []);
      const iGoals = filterDeleted(safeEntry.investmentGoals || safeEntry.investorGoals || []);
      ...
      return res.json({
        ...
        data: {
          ...
          investmentGoals: iGoals.map((g: any) => ({ ...g, userId: rawUserId })),
          investorGoals: iGoals.map((g: any) => ({ ...g, userId: rawUserId })),
          budgetGoals: safeEntry.budgetGoals || { essentials: 50, lifestyle: 30, investment: 20 },
          member_permissions: safeEntry.member_permissions || {},
  ...
  app.post('/api/data/sync', (req, res) => {
    try {
      const { userId, accounts, categories, familyMembers, transactions, financialGoals, investmentGoals, goals, budgetGoals, member_permissions, deletedIds } = req.body || {};
      if (!userId) {
        return res.status(400).json({ success: false, message: 'userId é obrigatório.' });
      }
      ...
      // SMART CATEGORY MERGE WITH RECURSIVE SUBCATEGORY PROTECTION
      const mergeCategories = (existingList: any[] = [], incomingList: any[] = []) => {
        const map = new Map<string, any>();
        (existingList || []).forEach((item: any) => {
          if (item && item.id && !deletedSet.has(item.id)) {
            const sanitized = { ...item, userId: canonicalId };
            delete sanitized._pendingSync;
            map.set(normalizeCatIdServer(item.id), sanitized);
          }
        });
        (incomingList || []).forEach((item: any) => {
          if (item && item.id && !deletedSet.has(item.id)) {
            const key = normalizeCatIdServer(item.id);
            const prev = map.get(key);
            const sanitized = { ...item, userId: canonicalId };
            delete sanitized._pendingSync;
            if (!prev) {
              ...
              const prevTime = prev.updatedAt ? new Date(prev.updatedAt).getTime() : 0;
              const incomingTime = sanitized.updatedAt ? new Date(sanitized.updatedAt).getTime() : 0;
              const mergedSubs = mergeSubcategoryTreesServer(prev.subcategories || [], sanitized.subcategories || [], deletedSet);
              const newer = incomingTime >= prevTime ? sanitized : prev;
              map.set(key, {
                ...prev,
                ...sanitized,
                name: newer.name || prev.name,
                color: newer.color || prev.color,
                icon: newer.icon || prev.icon,
                ruleGroup: newer.ruleGroup || prev.ruleGroup,
                type: newer.type || prev.type,
                subcategories: mergedSubs,
                userId: canonicalId,
                updatedAt: new Date(Math.max(prevTime, incomingTime, Date.now())).toISOString(),
              });
            }
          }
        });
      ...
      const syncedData: any = {
        ...
        financialGoals: mergeRecords(existing.financialGoals || existing.goals, financialGoals || goals),
        goals: mergeRecords(existing.financialGoals || existing.goals, financialGoals || goals),
        investmentGoals: mergeRecords(existing.investmentGoals || existing.investorGoals, investmentGoals),
        investorGoals: mergeRecords(existing.investmentGoals || existing.investorGoals, investmentGoals),
        budgetGoals: budgetGoals || existing.budgetGoals || { essentials: 50, lifestyle: 30, investment: 20 },
        member_permissions: { ...((existing as any).member_permissions || {}), ...(member_permissions || {}) },
  ...
  app.post('/api/data/transactional-goal', async (req, res) => {
    try {
      ...
      try {
        ...
        console.warn('[Server Sync Notice]', cloudErr);
      }
  ...
  app.post('/api/data/transactional-structure', async (req, res) => {
    try {
      ...
      // Merge incoming categoriesList if supplied so categories are never lost or wiped out
      if (Array.isArray(categoriesList) && categoriesList.length > 0) {
        ...
        (existing.categories || []).forEach((c: any) => {
          if (c && c.id && !deletedSet.has(c.id)) {
            const prev = catMap.get(c.id);
            if (!prev) {
              catMap.set(c.id, { ...c, userId: canonicalId });
            } else {
              const prevTime = prev.updatedAt ? new Date(prev.updatedAt).getTime() : 0;
              ...
              if (incomingTime > prevTime) {
                catMap.set(c.id, { ...prev, ...c });
              }
            }
          }
        });
        existing.categories = Array.from(catMap.values());
      }
      ...
      if (action === 'addCategory' || action === 'updateCategory') {
        ...
        const cIdx = existing.categories.findIndex((c: any) => isSameCategoryServer(c.id, sanitizedCat.id));
        if (cIdx >= 0) {
          const mergedSubs = mergeSubcategoryTreesServer(existing.categories[cIdx].subcategories || [], sanitizedCat.subcategories || []);
          existing.categories[cIdx] = { ...existing.categories[cIdx], ...sanitizedCat, subcategories: sanitizedCat.subcategories && sanitizedCat.subcategories.length > 0 ? sanitizedCat.subcategories : mergedSubs };
        } else {
        ...
        existing.categories = existing.categories.filter((c: any) => !isSameCategoryServer(c.id, idToDelete));
        ...
        let cIdx = existing.categories.findIndex((c: any) => isSameCategoryServer(c.id, catId));
        // If category is not in existing.categories, check if categoryData was provided to add it
        if (cIdx < 0 && categoryData) {
          const sanitizedCat = {
            ...categoryData,
            id: String(categoryData.id),
            userId: canonicalId,
          ...
          delete sanitizedCat._pendingSync;
          existing.categories.push(sanitizedCat);
          cIdx = existing.categories.length - 1;
        }

        if (cIdx >= 0) {
          const newSub = {
            id: String(subData.id || `sub_${Date.now()}_${Math.random().toString(36).substr(2, 4)}`),
            categoryId: existing.categories[cIdx].id,
        ...
        let cIdx = existing.categories.findIndex((c: any) => isSameCategoryServer(c.id, catId));
        if (cIdx < 0 && categoryData) {
          existing.categories.push({ ...categoryData, userId: canonicalId, updatedAt: new Date().toISOString() });
          cIdx = existing.categories.length - 1;
        }
        if (cIdx >= 0) {
      ...
      try {
        ...
        console.warn('[Server Sync Notice for Structure]', cloudErr);
      }
  ...
  app.post('/api/data/reset', (req, res) => {
    try {
      ...
      // Verificação estrita de titularidade
      if (requestingUserId) {
        const canonicalReq = getCanonicalUserIdServer(requestingUserId);
        if (canonicalReq !== canonicalId && requestingUserId !== userId) {
          const budgets = loadServerBudgets();
          const targetBudget = budgets.find((b: any) => 
            b.budgetId === canonicalId || 
            b.budgetId === userId || 
            (b.ownerEmail && b.ownerEmail.toLowerCase() === userId.toLowerCase())
          );
          const isOwner = targetBudget && (
            targetBudget.ownerId === canonicalReq ||
            targetBudget.ownerId === requestingUserId ||
            (targetBudget.ownerEmail && targetBudget.ownerEmail.toLowerCase() === requestingUserId.toLowerCase())
          );
          if (!isOwner) {
            console.warn('[API Data Reset] Unauthorized reset attempt by non-owner:', { requestingUserId, targetId: canonicalId });
            return res.status(403).json({ success: false, message: 'Ação bloqueada: Apenas o titular pode zerar o orçamento.' });
          }
        }
      }
  ...
  app.post('/api/portfolio/transactional-transaction', async (req, res) => {
    try {
      ...
      // Propagate directly to user financial document
      try {
        ...
        console.warn('[Server Sync Notice for Investment Transaction]', cloudErr);
      }
  ...
  app.post('/api/portfolio/transactional-allocations', async (req, res) => {
    try {
      ...
      try {
        ...
        console.warn('[Server Sync Notice for Target Allocations]', cloudErr);
      }
  ...
  app.post('/api/financials/transactional-budget-goals', async (req, res) => {
    try {
      ...
      try {
        ...
        console.warn('[Server Sync Notice for Budget Goals]', cloudErr);
      }
  ...
  app.post('/api/financials/transactional-gamification', async (req, res) => {
    try {
      ...
      // 3. Propagate directly to user financial document
      try {
        const portfolio = loadServerPortfolio();
        const userPort = portfolio[canonicalId] || { assets: [], transactions: [], targetAllocations: [] };
        const fullPayload = {
        ...
        console.warn('[Server Sync Notice for Gamification]', cloudErr);
      }
  ...
  // POST /api/financials/update-shared-requests
  app.post('/api/financials/update-shared-requests', (req, res) => {
    try {
      const { email, shared_requests } = req.body;
      if (!email) {
        return res.status(400).json({ success: false, message: 'Email required' });
      }
      const canonicalId = email.toLowerCase().trim();
      const financials = loadServerFinancials();
      let record: any = financials[canonicalId] || { accounts: [], categories: [], familyMembers: [], transactions: [], goals: [], deletedIds: [] };
      if (!record.shared_requests) record.shared_requests = [];
      record.shared_requests = shared_requests || [];
      financials[canonicalId] = record;
      saveServerFinancials(financials);
