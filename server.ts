...
async function startServer() {
    ...
    try {
      ...
      return res.json({
        ...
        data: {
          ...
          goals: filterDeleted(safeEntry.goals).map((g: any) => ({ ...g, userId: rawUserId })),
  ...
  app.post('/api/data/sync', (req, res) => {
    try {
      const { userId, accounts, categories, familyMembers, transactions, goals, budgetGoals, member_permissions, deletedIds } = req.body || {};
      if (!userId) {
        return res.status(400).json({ success: false, message: 'userId é obrigatório.' });
      }
