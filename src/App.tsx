...
export default function App() {
  ...
  const currentEffectiveBudgetId = useMemo(() => currentUser ? StorageService.getEffectiveBudgetId(currentUser) : 'default', [currentUser?.id]);
  ...
  useEffect(() => {
    ...
    const unsubscribeAppwrite = subscribeToAppwriteRealtime(activeBudgetId, (remoteData) => {
      if (remoteData) {
        ...
        if (remoteData.investorPortfolio && JSON.stringify(remoteData.investorPortfolio) !== JSON.stringify(investorPortfolio)) {
          const budgetId = StorageService.getEffectiveBudgetId(currentUser);
  ...
  const executeAppwriteDocumentUpdate = async (mutatedDataPayload: Partial<any>): Promise<boolean> => {
    ...
    try {
      ...
      // 3. Monta o payload respeitando TODOS os atributos do schema exatamente como no modal de membros
  ...
  const handleSaveUserName = (newName: string, avatarUrl?: string, city?: string, state?: string) => {
    if (!currentUser) return;
    ...
    if (updated) {
      updated.city = merged.city;
      updated.state = merged.state;
