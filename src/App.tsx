...
export default function App() {
  ...
  // Immediate local data population when currentUser changes to prevent white screen
  useEffect(() => {
    if (currentUser) {
  ...
  const checkReadOnlyPermission = (silent: boolean = false): boolean => {
    if (currentUser && StorageService.isCurrentUserReadOnly(currentUser)) {
      if (!silent) alert("Ação bloqueada: Você possui apenas permissão de LEITURA neste orçamento.");
      return true;
    }
    return false;
  };
