...
import {
  ...
  getAppwriteRealDocument,
  recordTransactionDeletion,
  filterOutDeletedTransactions,
...
export default function App() {
  ...
  const currentEffectiveBudgetId = useMemo(() => currentUser ? StorageService.getEffectiveBudgetId(currentUser) : 'default', [currentUser]);
  ...
  const mutationDebounceTimerRef = useRef<any>(null);

  const handleDataMutationDebounced = useCallback((e: Event, currentUser: User | null) => {
    if (mutationDebounceTimerRef.current) clearTimeout(mutationDebounceTimerRef.current);
    
    mutationDebounceTimerRef.current = setTimeout(async () => {
      if (currentUser) {
        const detail = (e as CustomEvent)?.detail;
        const bId = StorageService.getEffectiveBudgetId(currentUser);
        
        // 1. Update UI state from StorageService
        const newAccounts = StorageService.getAccounts(bId);
        const newCategories = StorageService.getCategories(bId);
        const newTransactions = StorageService.getTransactions(bId);
        const newGoals = StorageService.getGoals(bId);
        const newFamily = StorageService.getFamilyMembers(bId);
        ...
        // 2. TRIGGER CLOUD SYNC (Autosave)
        if (!detail || !detail.preventCloudSync) {
          console.log('[AUTOSAVE] Sincronizando alterações locais com o Appwrite...');
          const currentState = {
          ...
          const success = await persistCurrentStateToAppwrite(bId, currentState);
          setSyncStatus(success ? 'synced' : 'error');
          if (success) {
            console.log('[AUTOSAVE] Sucesso ao sincronizar.');
          }
        }
      }
  ...
  const resolveCanonicalAppwriteDocument = async (): Promise<string | null> => {
    ...
    try {
      ...
      let res;
      let retries = 2; // Reduced retries to avoid spamming
      let delay = 1000;
      while (retries > 0) {
        try {
          if (emailSanitizado) {
          ...
          retries--;
          if (retries === 0) {
             // If it's a network error, don't throw, just log quietly.
             if (e.message?.includes('Failed to fetch')) {
               console.warn(`[DEDO-DURO] Falha de rede ao resolver documento, usando fallback.`);
               return 'user_default';
             }
             throw e;
          }
          console.warn(`[SYNC AVISO] Falha ao resolver documento, tentando novamente em ${delay}ms (${retries} tentativas restantes)...`, e);
  ...
  const persistFinancialMutation = async (nextDataState: any): Promise<boolean> => {
    ...
    try {
      ...
      try {
        ...
        if (latestDoc && latestDoc.data) {
          ...
          finalDataState = {
            ...remoteState,
            ...nextDataState,
            transactions: mergeArrays(nextDataState.transactions, remoteState.transactions),
            accounts: mergeArrays(nextDataState.accounts, remoteState.accounts),
        ...
        console.warn('[SYNC] Erro ao buscar versão remota para merge, prosseguindo com overwrite seguro.', e);
      }
      ...
      await databases.updateDocument(
        ...
        resolvedDocId,
        {
          data: JSON.stringify(finalDataState)
        }
      );
  ...
  /**
   * 2. WEBSOCKET REALTIME UNIVERSAL ATIVO (Budget & Notifications)
   */
  useEffect(() => {
    ...
    async function initRealtimeSubscriptions() {
      ...
      try {
        ...
        if (resolvedDocId) {
          const budgetChannel = `databases.${DATABASE_ID}.collections.user_financials.documents.${resolvedDocId}`;
          console.log(`[REALTIME] Inscrevendo no orçamento:`, budgetChannel);

          unsubscribeBudget = client.subscribe(budgetChannel, (response: any) => {
            if (response.events.some((e: string) => e.includes('.update') || e.includes('.create'))) {
              ...
              if (docPayload && docPayload.data) {
                ...
                } catch (e) { remoteData = docPayload.data; }
        ...
        if (userEmail) {
          const notifChannel = `databases.${DATABASE_ID}.collections.Notificacoes.documents`;
          console.log(`[REALTIME] Inscrevendo em Notificações para:`, userEmail);

          unsubscribeNotifs = client.subscribe(notifChannel, (response: any) => {
            const payload = response.payload;
            if (payload && payload.userId?.toLowerCase().trim() === userEmail) {
              console.log(`[REALTIME] Nova notificação recebida!`, payload);
              loadInvites(); // Refresh pending invites list
              
              // If it's a request, maybe show a toast
              if (payload.tipo === 'request') {
                window.dispatchEvent(new CustomEvent('app-toast', { detail: 'Nova solicitação de acesso recebida!' }));
              }
            }
          });
        }
      } catch (err) {
        console.warn(`[REALTIME] Falha ao inicializar:`, err);
      }
    }
    ...
    return () => {
      if (unsubscribeBudget) try { unsubscribeBudget(); } catch (e) {}
      if (unsubscribeNotifs) try { unsubscribeNotifs(); } catch (e) {}
    };
  }, [currentUser?.email, currentEffectiveBudgetId]);
  ...
  useEffect(() => {
    if (currentUser) {
      loadInvites();
      // Periodically refresh invites every 30 seconds for maximum reliability
      const interval = setInterval(loadInvites, 30000);
      return () => clearInterval(interval);
    }
  }, [currentUser?.id, currentEffectiveBudgetId]);
  ...
  const loadInvites = async () => {
    try {
      ...
      // 1. Fetch from user_financials (old method)
      try {
      ...
      // 2. Fetch from Notificacoes (new method)
      try {
        ...
        console.warn('[loadInvites Notificacoes fetch error]', eNotif);
      }

      if (Array.isArray(pending) && pending.length > 0) {
        setPersistentInvites((prev) => JSON.stringify(pending) !== JSON.stringify(prev) ? pending : prev);
      } else {
        setPersistentInvites([]);
      }
    } catch (err) {
  ...
  const handleAcceptInvite = async (invite: any) => {
    ...
    try {
      ...
      const docIdCorreto = await resolveCanonicalAppwriteDocument();
      if (!docIdCorreto) throw new Error("Não foi possível localizar o documento do seu perfil financeiro.");
      ...
      if (inviteType === 'INVITE') {
        ...
        try {
          ...
          try {
            ownerDoc = await databases.getDocument(DATABASE_ID, 'user_financials', ownerDocId);
          } catch (e) {
            const ownerRes = await databases.listDocuments(DATABASE_ID, 'user_financials', [Query.equal('userId', ownerDocId)]);
            if (ownerRes.documents.length > 0) ownerDoc = ownerRes.documents[0];
          }
          ...
          if (ownerDoc) {
            ...
            await databases.updateDocument(
              ...
              'user_financials',
              ownerDoc.$id,
              { userId: ownerDoc.userId || ownerDoc.$id, data: JSON.stringify(ownerParsed) }
            );
          }
        } catch (ownerErr) {
        ...
        // Deletar da coleção Notificacoes (se existir)
        if (invite.id && !invite.id.startsWith('req_')) {
          try {
            await databases.deleteDocument(DATABASE_ID, 'Notificacoes', invite.id);
          } catch (eNotif) {}
        }
        ...
        const emailMembro = (invite.from_email || invite.emailRemetente || invite.from || invite.email || invite.remetente || '').toLowerCase().trim();
        if (!emailMembro) {
        ...
        json.pedidos_acesso = (json.pedidos_acesso || json.pending_invites || []).filter((p: any) => {
            const mail = (p.from_email || p.emailRemetente || p.from || p.email || p.remetente || '').toLowerCase().trim();
            return mail !== emailMembro && p.id !== invite.id;
        });

        await databases.updateDocument(DATABASE_ID, 'user_financials', doc.$id, { userId: currentUser.email, data: JSON.stringify(json) });

        // Update target member document to set active_budget_owner
        try {
            const docIdConvidado = getCanonicalAppwriteDocId(emailMembro);
            let targetDoc: any = null;
            try {
              targetDoc = await databases.getDocument(DATABASE_ID, 'user_financials', docIdConvidado);
            } catch (e) {
              const targetRes = await databases.listDocuments(DATABASE_ID, 'user_financials', [Query.equal('userId', emailMembro)]);
              if (targetRes.documents.length > 0) targetDoc = targetRes.documents[0];
            }

            if (targetDoc) {
              const jsonConvidado = targetDoc.data ? (typeof targetDoc.data === 'string' ? JSON.parse(targetDoc.data) : targetDoc.data) : {};
              ...
              // Se o convidado ainda não tem um orçamento ativo definido, define este como padrão
              if (!jsonConvidado.active_budget_owner) {
                jsonConvidado.active_budget_owner = doc.$id;
              }
              
              await databases.updateDocument(DATABASE_ID, 'user_financials', targetDoc.$id, { userId: emailMembro, data: JSON.stringify(jsonConvidado) });
            }
        } catch (err) {
  ...
  const handleRejectInvite = async (invite: any) => {
    ...
    try {
      const cfg = getAppwriteConfig();
      const DATABASE_ID = cfg.databaseId;
      const docId = await resolveCanonicalAppwriteDocument();
      if (!docId) throw new Error("Documento não encontrado.");

      const doc = await databases.getDocument(DATABASE_ID, 'user_financials', docId);
      let parsed = doc && doc.data ? (typeof doc.data === 'string' ? JSON.parse(doc.data) : doc.data) : {};

      const existingInvites = parsed.pedidos_acesso || parsed.pending_invites || [];
      parsed.pedidos_acesso = existingInvites.filter((inv: any) => {
        const invId = inv.id;
        const fromEmail = (inv.from_email || inv.emailRemetente || inv.from || inv.email || '').toLowerCase().trim();
        return invId !== invite.id && inv.budget_owner_id !== invite.budget_owner_id && fromEmail !== invite.from_email;
      });
      ...
      window.dispatchEvent(new CustomEvent('app-alert', { detail: { message: `Erro ao recusar: ${err.message || err}` } }));
    } finally {
      setLoadingInviteId(null);
    }
  };
  ...
  const refreshData = useCallback(async (user: User | null, forceRemote: boolean = true) => {
      ...
      try {
        ...
        if (session && session.email) {
          ...
          if (regCheck.exists && regCheck.user) {
            // Verifica/Provisiona documento no banco
            const cfg = getAppwriteConfig();
            ...
            try {
              ...
              if (res.documents && res.documents.length > 0) {
                userDoc = res.documents[0];
              }
            } catch (e) {
              console.error('[AUTO-PROVISIONAMENTO] Erro ao listar documentos:', e);
            }

            if (!userDoc) {
              console.log('[AUTO-PROVISIONAMENTO] Criando documento no banco para:', cleanEmail);
              ...
              const docPayload: Record<string, any> = {
                ...
                cidade: 'Brasília',
                estado: 'DF',
                data_nascimento: '1995-01-27T00:00:00.000Z',
                renda_mensal: 0,
                consent_lgpd: true,
              ...
              try {
                ...
                console.log('[AUTO-PROVISIONAMENTO SUCESSO] Criado ID:', userDoc.$id);
              } catch (createErr: any) {
                console.error('[AUTO-PROVISIONAMENTO ERRO]', createErr);
              }
            }
          } else {
          ...
          if (localStorage.getItem('darla_oauth_pending') === 'true') {
            setShowOpenAppModal(true);
            localStorage.removeItem('darla_oauth_pending');
          }
  ...
  useEffect(() => {
    if (currentUser) {
      const bId = StorageService.getEffectiveBudgetId(currentUser);
      if (bId !== prevBudgetId.current) {
        prevBudgetId.current = bId;
        ...
        if (bId) {
          setAccounts(prev => JSON.stringify(prev) === JSON.stringify(StorageService.getAccounts(bId)) ? prev : StorageService.getAccounts(bId));
          setCategories(prev => JSON.stringify(prev) === JSON.stringify(StorageService.getCategories(bId)) ? prev : StorageService.getCategories(bId));
          setTransactions(prev => JSON.stringify(prev) === JSON.stringify(StorageService.getTransactions(bId)) ? prev : StorageService.getTransactions(bId));
          setGoals(prev => JSON.stringify(prev) === JSON.stringify(StorageService.getGoals(bId)) ? prev : StorageService.getGoals(bId));
          setFamilyMembers(prev => JSON.stringify(prev) === JSON.stringify(StorageService.getFamilyMembers(bId)) ? prev : StorageService.getFamilyMembers(bId));
  ...
  const handleDataMutation = useCallback((e: Event) => {
    handleDataMutationDebounced(e, currentUser);
  }, [handleDataMutationDebounced, currentUser]);
  ...
  }, [handleBudgetUpdate, handleDataMutation]);
  ...
  const syncFn = useCallback(async () => {
    ...
    // Check if ID truly changed
    if (lastSyncedUserIdRef.current === currentUser.id) return;
    ...
    try {
      ...
      if (currentUser.email) {
        try {
          ...
          console.error('[SYNC] Failed to fetch user data:', e);
        }
      }
      const budgetId = StorageService.getEffectiveBudgetId(currentUser);
  ...
  }, [currentUser?.email, currentUser?.id]); // Use primitive ID/email instead of full user object

  const handleFocus = useCallback(() => {
    syncFn();
    refreshData(currentUser, true);
  }, [syncFn, currentUser?.id]);
  ...
  useEffect(() => {
    ...
    const unsubscribeAppwrite = subscribeToAppwriteRealtime(activeBudgetId, (remoteData) => {
      if (remoteData) {
        ...
        const currentAssets = PortfolioStorageService.getAssets(activeBudgetId);
        if (remoteData.investorPortfolio && JSON.stringify(remoteData.investorPortfolio) !== JSON.stringify(currentAssets)) {
          const budgetId = StorageService.getEffectiveBudgetId(currentUser);
          (PortfolioStorageService as any).saveToAllAliasKeys('darla_portfolio_assets', budgetId, remoteData.investorPortfolio);
        }
        const incomingInvGoals = remoteData.investmentGoals || remoteData.investorGoals;
        if (incomingInvGoals && Array.isArray(incomingInvGoals) && JSON.stringify(incomingInvGoals) !== JSON.stringify(investmentGoals)) {
          const budgetId = StorageService.getEffectiveBudgetId(currentUser);
        ...
        if (incomingFinancialGoals) {
          const mergedFinancialGoals = mergeRemoteGoalsWithOptimistic(incomingFinancialGoals);
          setFinancialGoals((prev) => JSON.stringify(mergedFinancialGoals) !== JSON.stringify(prev) ? mergedFinancialGoals : prev);
          StorageService.setGoals(mergedFinancialGoals as any, activeBudgetId);
        }
        if (remoteData.categories && Array.isArray(remoteData.categories) && JSON.stringify(remoteData.categories) !== JSON.stringify(categories)) {
        ...
        if (incomingMembers && Array.isArray(incomingMembers) && JSON.stringify(incomingMembers) !== JSON.stringify(familyMembers)) {
  ...
  }, [currentUser?.id, currentUser?.email, syncFn]);


    let remoteUpdateDebounceTimer: any = null;
    const handleRemoteUpdate = useCallback((evt?: any) => {
      ...
      if (detail?.categories && Array.isArray(detail.categories) && detail.categories.length > 0) {
        ...
        setCategories(prev => JSON.stringify(prev) === JSON.stringify(mappedCats) ? prev : mappedCats);
    ...
    const handleSessionRevoked = useCallback((_evt: any) => {
  ...
  useEffect(() => {
    ...
    handleFocus,
    handleRemoteUpdate,
    handleSharedBudgetUpdated,
    handleSharedBudgetsListUpdated,
    handleUserDeleted,
  ...
  const handleResetBudgetToZero = async () => {
    if (currentUser) {
      ...
      // Limpeza imediata local para refletir zero na UI instantaneamente (preservando dados do usuário logado)
      const savedUser = currentUser;
      const authKeys = ['cookie', 'session', 'appwrite', 'user', 'darla_current_user', 'currentuser'];
      ...
      if (savedUser) {
        localStorage.setItem('darla_current_user', JSON.stringify(savedUser));
  ...
  const executeAppwriteDocumentUpdate = async (mutatedDataPayload: Partial<any>): Promise<boolean> => {
    if (isReadOnly) {
      window.dispatchEvent(new CustomEvent('app-alert', { 
        detail: { message: "Ação bloqueada: Você possui apenas permissão de LEITURA neste orçamento compartilhado." } 
      }));
      return false;
    }
    ...
    try {
      ...
      // 1. Localiza o documento real ativo (se for membro conectado ao orçamento do titular, busca o documento do titular)
      const realDoc = await getAppwriteRealDocument(DATABASE_ID, COLLECTION_ID);
      if (!realDoc) {
        console.error('[DEDO-DURO] Nenhum documento retornado do Appwrite para atualizar.');
        return false;
      }

      // 2. Extrai e mescla o JSON interno do campo data
      ...
      // Purga metas órfãs de familyBudget para nunca mais ressuscitarem
      if (Array.isArray(nextDataObj.familyBudget)) {
        nextDataObj.familyBudget = nextDataObj.familyBudget.filter((item: any) => item.targetAmount === undefined && item.targetDate === undefined);
      }

      const docAny = realDoc as any;
      const fullAppwritePayload: Record<string, any> = {
        userId: docAny.userId,
        data: JSON.stringify(nextDataObj),
      ...
      // 4. Executa a gravação física no documento real usando o $id legítimo
      await databases.updateDocument(
        DATABASE_ID,
        COLLECTION_ID,
        realDoc.$id,
      ...
      console.log(`[DEDO-DURO SUCESSO] Gravado no documento Appwrite ($id: ${realDoc.$id})`);
      window.dispatchEvent(new Event('remote_data_updated'));
      window.dispatchEvent(new CustomEvent('financial_data_mutated'));
      return true;
    } catch (err: any) {
      console.error('[DEDO-DURO ERRO GRAVAÇÃO]', err);
      window.dispatchEvent(new CustomEvent('sync-error', { 
        detail: `Falha ao salvar no banco: ${err.message || JSON.stringify(err)}` 
      }));
      return false;
    }
  };
  ...
  const syncCurrentStateToCloud = async (overrideTxs?: Transaction[], overrideAccounts?: Account[]) => {
    try {
      ...
      await executeAppwriteDocumentUpdate(fullState);
    } catch (e) {
  ...
  const handleDeleteTransaction = async (id: string): Promise<boolean> => {
    ...
    console.log('[DEDO-DURO INÍCIO] Solicitada exclusão do ID:', id);
  ...
  const persistAllData = async (updatedAccounts?: any[], updatedTransactions?: any[], updatedInvestmentTransactions?: any[]) => {
    ...
    console.log(`[DEDO-DURO] persistAllData executado`, {
      accountsCount: accountsToPersist.length,
      transactionsCount: transactionsToPersist.length,
      budgetId,
    ...
    // 2. Cloud Persistence to Appwrite using canonical userIdentifier with DEDO-DURO
    try {
      ...
      console.log(`[DEDO-DURO] Sincronizando estado completo com Appwrite para:`, { userIdentifier, transactionsCount: transactionsToPersist.length });
      const success = await saveAppData(userIdentifier, fullState);
      if (!success) {
        console.warn('[DEDO-DURO] Appwrite sync retornou false.');
        setGlobalAlert({
          isOpen: true,
          title: 'Falha de Sincronização na Nuvem',
          message: 'O Appwrite retornou falha ao persistir os dados financeiros.',
          type: 'error'
        });
        return false;
      } else {
        console.log('[DEDO-DURO] Sucesso: Dados salvos e sincronizados no Appwrite para:', userIdentifier);
      }
    } catch (appwriteErr: any) {
      console.error('[DEDO-DURO] Exceção ao persistir no Appwrite:', appwriteErr);
      setGlobalAlert({
        isOpen: true,
        title: 'Falha de Sincronização na Nuvem',
        message: `Exceção Appwrite: ${appwriteErr?.message || appwriteErr}`,
        type: 'error'
      });
      return false;
    }
    ...
    try {
      ...
      console.log('[DEDO-DURO] Sync server-side concluído com sucesso.');
    } catch (error: any) {
      console.warn('[DEDO-DURO] Aviso server-side sync:', error?.message || error);
    }
  ...
  const handleSaveUserName = (newName: string, avatarUrl?: string, city?: string, state?: string, incomeBracket?: string, monthlyIncome?: number) => {
    if (!currentUser) return;
    const identifier = currentUser.email || currentUser.id;
    const updated = StorageService.updateUserProfile(identifier, newName, avatarUrl, city, state, incomeBracket, monthlyIncome);
    const merged: User = { 
      ...currentUser, 
      name: newName, 
      ...
      incomeBracket: incomeBracket !== undefined ? incomeBracket : currentUser.incomeBracket,
      monthlyIncome: monthlyIncome !== undefined ? monthlyIncome : currentUser.monthlyIncome
    };
    if (updated) {
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] flex flex-col font-sans text-[#121212]">
      ...
      {showOpenAppModal && (
        <div className="fixed inset-0 z-[10000] bg-black/50 backdrop-blur-md flex items-center justify-center p-6 animate-in fade-in duration-300">
          <div className="bg-white w-full max-w-md rounded-[32px] shadow-2xl p-8 text-center space-y-8 animate-in zoom-in-95 duration-300 border border-neutral-100">
            <h3 className="text-xl sm:text-2xl font-bold text-neutral-900 leading-tight px-2">
              Abrir no aplicativo DINHEIRO SEM FILTRO?
            </h3>
            <p className="text-neutral-600 text-sm leading-relaxed px-2">
              Você concluiu sua autenticação com sucesso. Deseja abrir o aplicativo instalado ou continuar navegando pela web?
            </p>
            <div className="grid grid-cols-2 gap-4">
              <button
                ...
                className="py-4 px-2 rounded-2xl border border-neutral-300 text-neutral-700 font-bold text-sm hover:bg-neutral-50 transition active:scale-95 cursor-pointer shadow-sm bg-white"
              >
                Agora não
              </button>
              <button
                ...
                onClick={async () => {
                  ...
                  if (userId && secret) {
                    try {
                      await appwriteCompleteOAuthSession(userId, secret);
                    } catch (e) {}
                  }
                  const url = new URL(window.location.href);
                  url.searchParams.delete('userId');
