...
export default function App() {
  ...
  // AUTHENTICATION & LOGIN FLOW (BLINDADO)
  useEffect(() => {
    let mounted = true;
    ...
    // A chave mestra: verifica se deixamos a tag de "login pendente" antes de ir pro Google
    const isOAuthPending = localStorage.getItem('darla_oauth_pending') === 'true';

    // Limpa a URL se houver lixo de erro do OAuth
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('error')) {
      window.history.replaceState({}, document.title, window.location.pathname);
    }

    if (isExplicitLogout && !isOAuthPending) {
      setCurrentUser(null);
    ...
    const initAuthBackground = async () => {
      try {
        ...
        if (isOAuthPending) {
          // Atraso de 1.5s para garantir a gravação dos cookies no retorno
          await new Promise(r => setTimeout(r, 1500));
        }

        // Tenta buscar o usuário (com retentativas caso a rede esteja lenta)
        try {
          ...
          if (isOAuthPending) {
            for (let i = 0; i < 3; i++) {
              ...
              appwriteUserActive = await getAppwriteUser().catch(() => null);
              if (appwriteUserActive) break;
            }
          }
        }

        if (appwriteUserActive && appwriteUserActive.email) {
          // LOGIN BEM SUCEDIDO VIA APPWRITE COOKIE
          ...
          // BLOQUEIO DE COOKIE DETECTADO (Navegador bloqueou a sessão do Appwrite)
          if (mounted) {
            setShowCookieFallback(true);
            setIsAuthLoading(false);
          }
        } else if (savedUserInitial && savedUserInitial.email) {
          // Já logado anteriormente (Offline ou Retorno normal)
          if (mounted) {
  ...
  const handleFallbackLogin = async () => {
    ...
    try {
      const synced = await StorageService.ensureUserAndDataSyncedAsync(cleanEmail, undefined, name, undefined, 'google');
      if (synced) {
        localStorage.setItem('darla_current_user', JSON.stringify(synced));
        setCurrentUser(synced);
  ...
  // TELA DE FALLBACK SE O NAVEGADOR BLOQUEAR O COOKIE
    ...
    return (
      <div className="min-h-screen bg-[#121212] flex flex-col items-center justify-center p-4">
        <div className="bg-white p-6 sm:p-8 rounded-3xl max-w-sm w-full text-center space-y-5 shadow-2xl border-2 border-[#D4AF37] animate-in fade-in zoom-in-95">
          ...
          <h2 className="font-black text-xl text-[#121212] font-serif">Acesso de Segurança</h2>
          <p className="text-xs sm:text-sm text-gray-600 font-medium">
            Seu navegador bloqueou o login automático (comum no Safari ou Guia Anônima). Para acessar seu orçamento, confirme o e-mail que você selecionou no Google:
          </p>
        ...
        {(activeTab === 'portfolio' || activeTab.startsWith('portfolio:')) && (
          StorageService.isFeatureAllowed(currentUser, 'portfolio') ? (
            ...
            <div className="p-8 bg-[#18181B] border-2 border-[#D4AF37] rounded-3xl text-center space-y-4 my-8 shadow-2xl">
              ...
              <p className="text-sm text-gray-300 max-w-md mx-auto">A Carteira do Investidor é recurso exclusivo dos planos VIP. Faça a assinatura para ter acesso completo a análises de patrimônio, proventos, rentabilidade e composição de carteira.</p>
