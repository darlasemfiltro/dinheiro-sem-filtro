...
export default function App() {
    ...
    const unsubscribeAppwrite = subscribeToAppwriteRealtime(activeBudgetId, (remoteData) => {
      if (remoteData) {
        ...
        if (remoteData.goals && Array.isArray(remoteData.goals)) {
          const mergedGoals = mergeRemoteGoalsWithOptimistic(remoteData.goals);
          setGoals(mergedGoals);
          StorageService.setGoals(mergedGoals as any, activeBudgetId);
        }
  ...
  const buildAppFinancialState = (
    ...
    const investorGoals = PortfolioStorageService.getGoals(portfolioUserId);
  ...
  const deleteInvestmentTransaction = async (txId: string) => {
    ...
    PortfolioStorageService.deleteTransaction(txId, portfolioUserId);
    ...
    (async () => {
      try {
        const result = await executeTransactionalInvestmentTransaction(portfolioUserId, 'deleteInvestmentTransaction', {
          transactionId: txId,
        });
