...
import { subscribeToAppwriteRealtime, getAppwriteUser, appwriteSignOut, appwriteDatabases as databases, appwriteClient as client, getAppwriteConfig } from './lib/appwrite';
    ...
    try {
      ...
      if (inviteType === 'INVITE') {
        ...
        if (!emailMembro) {
          ...
          return alert("[ERRO] Falha ao extrair e-mail do convite.");
        }
    ...
    const initAuth = async () => {
      ...
      try {
        ...
        if (appwriteUser && appwriteUser.email && mounted) {
          ...
          const docIdEsperado = 'user_' + cleanEmail.replace(/@/g, '.').replace(/[^a-zA-Z0-9._-]/g, '_');
          
          console.log("🔍 Buscando cofre do usuário logado:", docIdEsperado);

          // Fail-safe immediate user initialization to prevent login loop
          ...
          refreshData(fallbackUser, false);
          setIsAuthLoading(false);

          // Attempt direct fetch from user_financials database with canonical document ID (fail-safe anti-crash)
          try {
            ...
            if (userData && mounted) {
              console.log("✅ Cofre financeiro encontrado na nuvem para:", docIdEsperado);
              let dadosFinanceiros: any = {};
              let convitesPendentes = [];
              
              try {
                if (userData && userData.data) {
                  dadosFinanceiros = typeof userData.data === 'string' 
                    ? JSON.parse(userData.data) 
                    : userData.data;
                  
                  convitesPendentes = dadosFinanceiros.shared_requests || [];
                }
              } catch (parseError) {
                console.error("❌ Erro fatal ao ler os dados:", parseError);
              }
              console.log("📬 Convites pendentes carregados:", convitesPendentes.length);
            }
          } catch (dbErr: any) {
            console.warn("⚠️ Cofre financeiro não encontrado ou erro de banco. Iniciando vazio sem ejeção:", dbErr?.message || dbErr);
          }

          // Separate DB synchronization in background (fail-safe)
          try {
            ...
            if (syncedUser && mounted) {
              setCurrentUser(syncedUser);
              refreshData(syncedUser, false);
            }
          } catch (dbErr) {
            console.error('[Database lookup error during session check - fail-safe fallback active]:', dbErr);
          }
          return;
        }
      } catch (e) {}
