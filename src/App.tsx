...
export default function App() {
  ...
  const handleDeleteTransaction = async (id: string): Promise<boolean> => {
    ...
    console.log(`[DEDO-DURO] Iniciando exclusão de transação(ões)`, {
      targetIds: idsToDelete,
      prevCount,
      nextCount: nextTransactions.length,
      currentUserEmail: currentUser?.email
    });

    // Atualizar imediatamente o estado do React e o Storage local
    setTransactions(nextTransactions);
    const budgetId = currentUser ? StorageService.getEffectiveBudgetId(currentUser) : 'default';
    StorageService.setTransactions(nextTransactions, budgetId);

    // Persistir de forma robusta no Appwrite utilizando o $id real do documento com DEDO-DURO
    try {
      const databaseId = '6a83aa8d0038331e040f';
      const collectionId = 'user_financials';
      const userIdentifier = budgetId || currentUser?.email || currentUser?.id || 'default';
      
      console.log(`[DEDO-DURO] Buscando documento real no Appwrite para exclusão...`, { userIdentifier });
      const realDoc = await getAppwriteRealDocument(databaseId, collectionId, userIdentifier);
      
      if (realDoc && realDoc.$id) {
        console.log(`[DEDO-DURO] Documento alvo resolvido no Appwrite ($id real):`, realDoc.$id);
        const parsedData = realDoc.data || {};
        ...
        console.log(`[DEDO-DURO] Sucesso HTTP Appwrite: Lançamento excluído fisicamente do documento ${realDoc.$id}`);
      } else {
        console.warn(`[DEDO-DURO] Documento real não localizado por getAppwriteRealDocument. Acionando persistAllData de fallback.`);
        const fallbackSuccess = await persistAllData(accounts, nextTransactions);
        if (!fallbackSuccess) {
          throw new Error('Falha ao gravar alteração na nuvem através do fallback.');
        }
      }
    } catch (err: any) {
      console.error(`[DEDO-DURO] Falha crítica na exclusão no Appwrite:`, err);
      setGlobalAlert({
        isOpen: true,
        title: 'Falha de Sincronização na Nuvem',
        message: `Não foi possível remover o lançamento no Appwrite: ${err?.message || err}`,
        type: 'error'
      });
      return false;
    }
