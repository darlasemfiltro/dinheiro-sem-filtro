 ...
 * preservando obrigatoriamente todos os atributos obrigatórios do Schema (estado, cidade, data_nascimento, renda_mensal, consent_lgpd).
 */
export async function executeAppwriteDocumentUpdate(targetDocId: string, updatedDataState: any): Promise<boolean> {
  ...
  try {
    ...
    const safePayload: Record<string, any> = {
      ...
      data_nascimento: currentDoc?.data_nascimento || '2000-01-01',
      renda_mensal: currentDoc?.renda_mensal ?? 0,
      consent_lgpd: currentDoc?.consent_lgpd ?? true
    };
  ...
  try {
    ...
    if (docs.length > 0) {
      ...
      const finalPayload = {
        ...
        estado: latestDoc.estado || 'DF',
        cidade: latestDoc.cidade || 'Brasília',
        data_nascimento: latestDoc.data_nascimento || '2000-01-01',
        renda_mensal: latestDoc.renda_mensal ?? 0,
        consent_lgpd: latestDoc.consent_lgpd ?? true
      };
...
export const saveAppDataDirect = async (targetUserOrBudgetId: string, fullDataPayload: any): Promise<boolean> => {
  try {
    ...
    if (!realDoc) {
      console.error('[SYNC ERRO] Documento não encontrado no Appwrite para atualizar.');
      return false;
    }
    ...
    const fullPayload: Record<string, any> = {
      ...
      cidade: realDoc.cidade || 'Brasília',
      estado: realDoc.estado || 'DF',
      data_nascimento: realDoc.data_nascimento || '1999-12-31T22:00:00.000Z',
      renda_mensal: realDoc.renda_mensal ?? 0,
      consent_lgpd: realDoc.consent_lgpd ?? false
    };
