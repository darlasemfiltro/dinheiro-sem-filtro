...
export async function safeCreateFinancialDocument(cleanEmail: string, formValues: any) {
  ...
  try {
    ...
    console.warn('[APPWRITE] Aviso ao buscar documento existente:', lookupErr);
  }

  if (existingDocId) {
    console.log('[APPWRITE] Documento user_financials já existente encontrado para', email, 'Atualizando ID:', existingDocId);
    try {
      ...
      console.warn('[APPWRITE] Falha na atualização, tentando payload reduzido...', updateErr);
    }
  }
  ...
  try {
    ...
    console.warn('[FALLBACK] Tentativa primária falhou. Tentando payload com cidade e estado...', errPrimario);
...
export async function appwriteGoogleOAuthLogin(successUrl?: string, failureUrl?: string) {
  ...
  try {
    if (cfg.projectId && cfg.projectId !== 'default-placeholder') {
      ...
      const oauthUrl = `${cfg.endpoint}/account/sessions/oauth2/google?project=${cfg.projectId}&success=${encodeURIComponent(targetSuccess)}&failure=${encodeURIComponent(targetFailure)}`;
      ...
      const popup = window.open(oauthUrl, 'google_oauth_login', `width=${width},height=${height},top=${top},left=${left}`);
      if (!popup) {
