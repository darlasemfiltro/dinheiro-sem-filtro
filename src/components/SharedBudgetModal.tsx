...
export const SharedBudgetModal: React.FC<SharedBudgetModalProps> = ({
    ...
    try {
      ...
      if (meuDoc) {
        ...
        const updatePayload: Record<string, any> = {
          ...
          data_nascimento: meuDoc.data_nascimento || '2000-01-01',
  ...
  const concederAcesso = async (emailConvidado: string) => {
    ...
    try {
      ...
      if (!shared.includes(meuEmail)) shared.includes(meuEmail) ? null : shared.push(meuEmail);
      ...
      const alvoPayload = {
        ...
        estado: docAlvo?.estado || 'DF',
        cidade: docAlvo?.cidade || 'Brasília',
        data_nascimento: docAlvo?.data_nascimento || '2000-01-01',
        renda_mensal: docAlvo?.renda_mensal ?? 0,
        consent_lgpd: docAlvo?.consent_lgpd ?? true
      };
      ...
      console.error(e);
      window.dispatchEvent(new CustomEvent('app-alert', { detail: { message: "Erro ao salvar no banco." } }));
    } finally {
      setIsLoading(false);
    }
  };
        ...
        onConfirm: async () => {
          try {
            try {
              ...
              console.warn("Appwrite cloud removal sync skipped/failed:", appwriteErr);
            }
            ...
            window.dispatchEvent(new CustomEvent('app-alert', { detail: { message: "Erro ao excluir membro." } }));
          }
        }
      }
    }));
