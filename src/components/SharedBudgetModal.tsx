...
export const SharedBudgetModal: React.FC<SharedBudgetModalProps> = ({
  ...
  useEffect(() => {
    const buscarDados = async () => {
      ...
      try {
        ...
        let json: any = {};
        if (docMe && docMe.data) {
          json = typeof docMe.data === 'string' ? JSON.parse(docMe.data) : docMe.data;
        }
        ...
        // Identifica orçamentos compartilhados com este usuário (Fonte de Verdade: shared_with_me)
        ...
        for (const ownerEmail of sharedWithMeList) {
          try {
            ...
            if (docOwner) {
              const dJson = typeof docOwner.data === 'string' ? JSON.parse(docOwner.data || '{}') : (docOwner.data || {});
              ...
              if (isStillAllowed) {
                ...
                // Se não está mais na lista do titular, remove do shared_with_me localmente e no doc do usuário
                sharedWithMeList = sharedWithMeList.filter(email => email.toLowerCase().trim() !== ownerEmailClean);
                json.shared_with_me = sharedWithMeList;
                if (docMe) {
                  await databases.updateDocument(config.databaseId, 'user_financials', docMe.$id, { data: JSON.stringify(json) });
                }
              }
            }
          } catch (err) {}
  ...
  const concederAcesso = async (emailConvidado: string) => {
    ...
    try {
      ...
      // Robustly find owner's document
      let meuDoc: any = null;
      try {
      ...
      // Robustly find target's document
      let docAlvo: any = null;
      try {
        const alvoDocId = getCanonicalAppwriteDocId(emailAlvo);
        docAlvo = await databases.getDocument(config.databaseId, 'user_financials', alvoDocId);
      } catch (e) {
        const listAlvo = await databases.listDocuments(config.databaseId, 'user_financials', [
          Query.equal('userId', emailAlvo),
          Query.limit(1)
        ]);
        if (listAlvo.documents.length > 0) docAlvo = listAlvo.documents[0];
      }
      ...
      jsonAlvo.active_budget_owner = meuDoc?.$id || getCanonicalAppwriteDocId(meuEmail);

      const alvoPayload = {
        userId: emailAlvo,
        data: JSON.stringify(jsonAlvo)
      };
      ...
      const myDocIdForNotif = meuDoc?.$id || getCanonicalAppwriteDocId(meuEmail);
      try {
        await databases.createDocument(config.databaseId, 'Notificacoes', ID.unique(), {
          userId: emailAlvo,
      ...
      const errorMsg = (e as any)?.message || JSON.stringify(e);
      window.dispatchEvent(new CustomEvent('app-alert', { 
        detail: { 
          message: `Erro ao salvar no banco: ${errorMsg.substring(0, 100)}` 
        } 
      }));
    } finally {
      setIsLoading(false);
      ...
      detail: {
        ...
        onConfirm: async () => {
          try {
            ...
            if (!docMe) throw new Error("Não foi possível localizar o seu documento de perfil para realizar a exclusão.");
            ...
            await databases.updateDocument(config.databaseId, 'user_financials', docMe.$id, { userId: meuEmail, data: JSON.stringify(jsonMe) });

            const emailAlvo = emailMembro.toLowerCase().trim();
            
            // Robustly find target member document
            let docConv: any = null;
            try {
              const idConvidado = getCanonicalAppwriteDocId(emailAlvo);
              docConv = await databases.getDocument(config.databaseId, 'user_financials', idConvidado);
            } catch (e) {
              const listRes = await databases.listDocuments(config.databaseId, 'user_financials', [
                Query.equal('userId', emailAlvo),
                Query.limit(1)
              ]);
              if (listRes.documents.length > 0) {
                docConv = listRes.documents[0];
              }
            }
            ...
            if (docConv) {
              ...
              // Also check if the active_budget_owner points to the owner's email or document ID
              if (
                jsonConv.active_budget_owner === meuEmail ||
                jsonConv.active_budget_owner === docMe.$id
              ) {
                jsonConv.active_budget_owner = null;
              }
              
              await databases.updateDocument(config.databaseId, 'user_financials', docConv.$id, { 
                userId: emailAlvo, 
                data: JSON.stringify(jsonConv) 
              });
              console.log(`[DEDO-DURO] Member ${emailAlvo} document updated (access revoked).`);
            }
            ...
            window.dispatchEvent(new CustomEvent('app-alert', { detail: { message: "Erro ao excluir membro no banco de dados." } }));
          }
        }
      }
    }));
