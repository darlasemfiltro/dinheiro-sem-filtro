...
export const GoalsView: React.FC<GoalsViewProps> = ({
  ...
  const [isPeriodUnitPickerOpen, setIsPeriodUnitPickerOpen] = useState(false);
  const [isYieldPeriodPickerOpen, setIsYieldPeriodPickerOpen] = useState(false);
                  ...
                  <div className="grid grid-cols-2 gap-2 pt-0.5">
                    ...
                    <div className="relative">
                      ...
                      <button
                        type="button"
                        onClick={() => {
                          setIsPeriodUnitPickerOpen(!isPeriodUnitPickerOpen);
                          setIsYieldPeriodPickerOpen(false);
                          setIsCategoryPickerOpen(false);
                        }}
                        className="w-full px-3 py-1.5 bg-white border border-gray-300 rounded-xl text-xs font-bold text-[#121212] flex items-center justify-between hover:bg-gray-50 transition cursor-pointer"
                      >
                        <span>{periodUnit === 'months' ? 'Mês / Meses' : 'Ano / Anos'}</span>
                        <ChevronDown className={`w-3.5 h-3.5 text-gray-500 transition-transform ${isPeriodUnitPickerOpen ? 'rotate-180' : ''}`} />
                      </button>

                      {isPeriodUnitPickerOpen && (
                        <div className="absolute left-0 right-0 mt-1 bg-[#182232] text-white rounded-2xl p-2.5 shadow-2xl border border-gray-700 space-y-1.5 z-30">
                          {[
                            { value: 'months', label: 'Mês / Meses' },
                            { value: 'years', label: 'Ano / Anos' },
                          ].map((opt) => {
                            const isSelected = periodUnit === opt.value;
                            return (
                              <div
                                key={opt.value}
                                onClick={() => {
                                  const unit = opt.value as 'months' | 'years';
                                  setPeriodUnit(unit);
                                  const computed = computeDateFromPeriod(periodValue, unit);
                                  setTargetDate(computed);
                                  setDateText(isoToBrDate(computed));
                                  setIsPeriodUnitPickerOpen(false);
                                }}
                                className={`w-full py-2.5 px-3 rounded-xl font-bold text-xs flex items-center justify-between transition cursor-pointer ${
                                  isSelected
                              ...
                              >
                                <span>{opt.label}</span>
                                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center transition ${isSelected ? 'border-[#00d2ff]' : 'border-gray-500'}`}>
                                  {isSelected && <div className="w-2 h-2 rounded-full bg-[#00d2ff]" />}
                                </div>
                              </div>
                            );
                          })}
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="relative">
                    ...
                    <button
                      type="button"
                      onClick={() => {
                        setIsYieldPeriodPickerOpen(!isYieldPeriodPickerOpen);
                        setIsPeriodUnitPickerOpen(false);
                        setIsCategoryPickerOpen(false);
                      }}
                      className="w-full px-3 py-1.5 bg-white border border-gray-300 rounded-xl text-xs font-bold text-[#121212] flex items-center justify-between hover:bg-gray-50 transition cursor-pointer"
                    >
                      <span className="pr-1 text-[11px]">
                        {yieldPeriod === 'monthly' ? 'Mensal (% ao Mês)' : 'Anual (% ao Ano)'}
                      </span>
                      <ChevronDown className={`w-3.5 h-3.5 text-gray-500 shrink-0 transition-transform ${isYieldPeriodPickerOpen ? 'rotate-180' : ''}`} />
                    </button>

                    {isYieldPeriodPickerOpen && (
                      <div className="absolute left-0 right-0 mt-1 bg-[#182232] text-white rounded-2xl p-2.5 shadow-2xl border border-gray-700 space-y-1.5 z-30">
                        {[
                          { value: 'monthly', label: 'Mensal (% ao Mês - CDI)' },
                          { value: 'yearly', label: 'Anual (% ao Ano - Tesouro)' },
                        ].map((opt) => {
                          const isSelected = yieldPeriod === opt.value;
                          return (
                            <div
                              key={opt.value}
                              onClick={() => {
                                setYieldPeriod(opt.value as 'monthly' | 'yearly');
                                setIsYieldPeriodPickerOpen(false);
                              }}
                              className={`w-full py-2.5 px-3 rounded-xl font-bold text-xs flex items-center justify-between transition cursor-pointer ${
                                isSelected
                            ...
                            >
                              ...
                              <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition ${isSelected ? 'border-[#00d2ff]' : 'border-gray-500'}`}>
                                {isSelected && <div className="w-2 h-2 rounded-full bg-[#00d2ff]" />}
                              </div>
                            </div>
                          );
