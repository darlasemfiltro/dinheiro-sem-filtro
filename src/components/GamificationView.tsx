...
export const GamificationView: React.FC<GamificationViewProps> = ({ userId }) => {
  ...
  const awardGamification = async (
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('app-toast', { detail: 'Erro ao processar recompensa. Estado anterior restaurado.' }));
    }
  };
  ...
  const handleBuyItem = async (itemType: 'freeze' | 'doubleXp', cost: number) => {
    if (gameState.gems < cost) {
      window.dispatchEvent(new CustomEvent('app-toast', { detail: 'Gemas insuficientes para esta compra.' }));
      return;
    }
    ...
    try {
      ...
      GamificationService.saveGamificationState(backupState);
      window.dispatchEvent(new CustomEvent('sync-status-change', { detail: 'error' }));
      window.dispatchEvent(new CustomEvent('app-toast', { detail: 'Erro ao processar a compra. Suas gemas foram devolvidas.' }));
    }
  };
    ...
    <div className="max-w-5xl mx-auto space-y-6 pb-12 animate-in fade-in duration-300">
      ...
      <div className="bg-gradient-to-r from-[#121212] via-[#1E1E1E] to-[#121212] border-2 border-[#D4AF37] rounded-3xl p-6 text-white shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col gap-6 relative z-10">
          {/* Top Row: Flame Icon & Main Title / Subtitle */}
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 text-center sm:text-left">
            <div className="relative shrink-0">
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-3xl bg-gradient-to-tr from-amber-500 via-orange-500 to-red-500 flex items-center justify-center shadow-xl border-2 border-amber-300">
                <Flame className="w-12 h-12 text-white animate-pulse" />
              </div>
              <span className="absolute -bottom-2 right-1/2 translate-x-1/2 sm:translate-x-0 sm:-right-2 bg-black text-[#FFD700] text-[10px] font-black px-2.5 py-0.5 rounded-full border border-[#D4AF37] shadow-sm whitespace-nowrap">
                SEMANAL
              </span>
            </div>

            <div className="space-y-2 flex-1">
              <div className="inline-flex items-center gap-1.5 text-xs font-black uppercase tracking-wider text-[#D4AF37] bg-black/60 px-3.5 py-1 rounded-full border border-[#D4AF37]/40 shadow-xs">
                <span>🔥 OFENSIVA FINANCEIRA SEMANAL</span>
              </div>
              ...
              <p className="text-xs sm:text-sm text-gray-300 max-w-xl">
                Você mantém seus lançamentos e orçamento atualizados semanalmente. Continue firme para subir de divisão e acumular recompensas!
              </p>
            </div>
          </div>
          ...
          {/* Bottom Row: Gamification Counters Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full">
            {/* XP Total */}
            <div className="bg-black/60 border border-white/10 rounded-2xl p-3 text-center shadow-inner hover:border-amber-400/40 transition">
              <div className="flex items-center justify-center gap-1.5 text-amber-400 mb-1">
                <Zap className="w-4 h-4" />
                <span className="text-[11px] font-black uppercase tracking-wide">XP Total</span>
              </div>
              <p className="text-base sm:text-lg font-black text-white">{gameState.xpTotal}</p>
            </div>

            {/* Gemas */}
            <div className="bg-black/60 border border-white/10 rounded-2xl p-3 text-center shadow-inner hover:border-cyan-400/40 transition">
              <div className="flex items-center justify-center gap-1.5 text-cyan-400 mb-1">
                <Gem className="w-4 h-4" />
                <span className="text-[11px] font-black uppercase tracking-wide">Gemas</span>
              </div>
              <p className="text-base sm:text-lg font-black text-cyan-300">💎 {gameState.gems}</p>
            </div>

            {/* Congelamento de Ofensiva */}
            <div className="bg-black/60 border border-blue-500/30 rounded-2xl p-3 text-center shadow-inner hover:border-blue-400/50 transition">
              <div className="flex items-center justify-center gap-1.5 text-blue-400 mb-1">
                <ShieldCheck className="w-4 h-4" />
                <span className="text-[11px] font-black uppercase tracking-wide">Congelamento</span>
              </div>
            ...
            {/* Divisão */}
            <div className="bg-black/60 border border-pink-500/30 rounded-2xl p-3 text-center shadow-inner hover:border-pink-400/50 transition">
              <div className="flex items-center justify-center gap-1.5 text-pink-400 mb-1">
                <Trophy className="w-4 h-4" />
                <span className="text-[11px] font-black uppercase tracking-wide">Divisão</span>
              </div>
              <p className="text-sm sm:text-base font-black text-pink-300 truncate">
          ...
          <div className="bg-gradient-to-b from-pink-950 via-[#1A0B18] to-[#121212] border-2 border-pink-500 rounded-3xl p-8 text-center text-white shadow-2xl space-y-4 relative overflow-hidden">
            ...
            <div className="space-y-3 max-w-xl mx-auto">
              ...
              {(() => {
                ...
                return (
                  <div className="mt-4 p-3 sm:p-5 bg-white/10 backdrop-blur-md border border-pink-400/40 rounded-2xl text-left space-y-3 text-xs shadow-inner overflow-hidden">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-pink-400/30 pb-3">
                      ...
                      <span className={`text-[10px] font-black px-2.5 py-1 rounded-full text-center sm:text-left self-start sm:self-auto shrink-0 ${
                    ...
                    <ul className="space-y-2 text-pink-100 font-medium leading-relaxed">
                      <li className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-2 bg-black/20 p-3 rounded-xl border border-white/5 overflow-hidden">
                        <span className="text-amber-400 font-black shrink-0">🎯 Meta da Divisão:</span>
                        <span className="break-words">{nextDiv.goal}</span>
                      </li>
                      <li className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-2 bg-black/20 p-3 rounded-xl border border-white/5 overflow-hidden">
                        <span className="text-emerald-400 font-black shrink-0">📌 Critério Financeiro Real:</span>
                        <div className="space-y-1 min-w-0 flex-1">
                          <p className="font-bold text-white break-words">{nextDiv.financialReq}</p>
                          {!qual.metFinancial && qual.reasonIfNotMet && (
                            <p className="text-[11px] text-rose-300 font-bold bg-rose-950/60 p-2 rounded-lg border border-rose-500/30 flex items-start sm:items-center gap-1.5 mt-1 break-words">
                              <AlertTriangle className="w-3.5 h-3.5 shrink-0 text-rose-400 mt-0.5 sm:mt-0" />
                              <span className="break-words">{qual.reasonIfNotMet}</span>
                            </p>
                          )}
                        </div>
                      </li>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-3 border-b border-gray-200 gap-2">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="inline-flex items-center gap-1 text-[11px] text-blue-700 font-extrabold bg-gradient-to-r from-blue-100 to-indigo-100 border border-blue-300 px-3 py-1 rounded-xl shadow-xs">
      ...
      {activeTab === 'loja' && (
        <div className="space-y-6 animate-in fade-in">
          <div className="bg-gradient-to-r from-emerald-900 to-teal-950 border-2 border-emerald-500 rounded-3xl p-6 text-white shadow-xl flex items-center justify-between gap-4">
            <div className="space-y-1">
              <span className="text-xs font-black text-emerald-300 uppercase">LOJA SEM FILTRO</span>
