...
export const AuthScreen: React.FC<AuthScreenProps> = ({ onLoginSuccess }) => {
  ...
  // Google Login Handler via Appwrite OAuth 2.0 with dynamic origin redirect
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      await appwriteGoogleOAuthLogin(currentUrl, currentUrl);
    } catch (err) {
      console.warn('[OAuth Redirect fallback to modal]', err);
  ...
  const handleConfirmGoogleLogin = async (e: React.FormEvent) => {
    ...
    if (!customGoogleEmail || !customGoogleEmail.includes('@')) {
      setError('Por favor, informe um e-mail do Google válido.');
      return;
    }
    setIsLoadingGoogle(true);
    setError('');
    const cleanEmail = customGoogleEmail.trim().toLowerCase();
    const displayName = customGoogleName.trim() || cleanEmail.split('@')[0];

    try {
      await appwriteSignUp(cleanEmail, 'GoogleAuth2026!Secure', displayName).catch(() => {});
      if (auth) {
        try {
          await signInWithEmailAndPassword(auth, cleanEmail, 'GoogleAuth2026!Secure').catch(async () => {
            await createUserWithEmailAndPassword(auth, cleanEmail, 'GoogleAuth2026!Secure').catch(() => {});
          });
        } catch {}
      }
  ...
  // Step 1: Send Password Reset Code by Email
  const handleSendResetCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setInputCode(''); // Clear input code so user manually types code received in their email inbox
  ...
  return (
    ...
    >
      ...
      >
        ...
        {mode === 'auth' && (
          <div className="space-y-4">
            ...
            {/* Tab Toggle */}
            <div className="flex bg-gray-100 p-1.5 rounded-2xl gap-1.5 border border-gray-200" id="auth-tabs">
              <button
                type="button"
                onClick={() => {
                  setIsRegister(false);
                ...
                className={`flex-1 py-3 sm:py-3.5 px-3 text-xs sm:text-sm font-black rounded-xl transition-all cursor-pointer truncate min-h-[44px] ${
            ...
            <div className="pt-4 mt-4 border-t border-gray-100 text-center">
              <button
                type="button"
                onClick={() => {
                  if (window.confirm('Tem certeza absoluta que deseja excluir todas as contas, zerar todos os dados e reiniciar o sistema do zero em todos os dispositivos?')) {
                    StorageService.wipeSystem();
                  }
                }}
                className="text-[11px] text-red-500 hover:text-red-700 font-bold hover:underline cursor-pointer"
              >
                ⚠️ Zerar todos os dados e reiniciar contas
              </button>
            </div>
          </div>
        )}
      ...
      {/* Google Quick Login Modal to prevent origin_mismatch */}
      {showGoogleModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in">
          <div className="bg-white border-2 border-[#D4AF37] rounded-3xl shadow-2xl max-w-md w-full p-6 space-y-5 relative">
            <button
              type="button"
              onClick={() => setShowGoogleModal(false)}
              className="absolute right-4 top-4 text-gray-400 hover:text-black p-1 cursor-pointer"
            >
              <X className="w-5 h-5" />
            </button>

            <div className="text-center space-y-1.5">
              <div className="w-12 h-12 bg-[#4285F4]/10 rounded-2xl flex items-center justify-center mx-auto text-[#4285F4] mb-2">
              ...
              <h3 className="text-lg font-black text-[#121212]">Entrar com Google</h3>
              <p className="text-xs text-gray-500">
                Para evitar bloqueios de autorização do Google em domínios de pré-visualização, digite seu e-mail do Google para acesso instantâneo e sincronizado.
              </p>
            </div>

            {/* Quick Select Buttons for User's tested emails */}
            <div className="space-y-2">
              <span className="text-[11px] font-bold text-gray-600 uppercase tracking-wider">Acesso rápido com um toque:</span>
              <div className="grid grid-cols-1 gap-2">
                <button
                  type="button"
                  onClick={() => {
                    setCustomGoogleEmail('carvalho.darlla@gmail.com');
                    setCustomGoogleName('Darla Carvalho');
                  }}
                  className={`p-2.5 rounded-xl border text-left text-xs font-bold transition flex items-center justify-between cursor-pointer ${
                    customGoogleEmail === 'carvalho.darlla@gmail.com' ? 'border-[#D4AF37] bg-[#D4AF37]/10 text-black' : 'border-gray-200 hover:bg-gray-50 text-gray-700'
                  }`}
                >
                  <span>carvalho.darlla@gmail.com</span>
                  <span className="text-[10px] text-gray-600">Conta Principal</span>
                </button>
                <button
                  type="button"
                  onClick={() => {
                    setCustomGoogleEmail('danilujb@gmail.com');
                    setCustomGoogleName('Danilu');
                  }}
                  className={`p-2.5 rounded-xl border text-left text-xs font-bold transition flex items-center justify-between cursor-pointer ${
                    customGoogleEmail === 'danilujb@gmail.com' ? 'border-[#D4AF37] bg-[#D4AF37]/10 text-black' : 'border-gray-200 hover:bg-gray-50 text-gray-700'
                  }`}
                >
                  <span>danilujb@gmail.com</span>
                  <span className="text-[10px] text-gray-600">Conta Secundária</span>
                </button>
              </div>
            </div>
            ...
            <form onSubmit={handleConfirmGoogleLogin} className="space-y-3 pt-2">
              <div className="space-y-1">
                <label className="text-xs font-bold text-[#121212]">E-mail do Google</label>
                <input
                  type="email"
                  value={customGoogleEmail}
                  onChange={(e) => setCustomGoogleEmail(e.target.value)}
                  placeholder="ex: carvalho.darlla@gmail.com"
                  className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-xs text-[#121212] focus:outline-none focus:ring-2 focus:ring-[#D4AF37]"
                  required
                />
              </div>
              <div className="space-y-1">
                <label className="text-xs font-bold text-[#121212]">Seu Nome</label>
                <input
                  type="text"
              ...
              <button
                type="submit"
                disabled={isLoadingGoogle}
                className="w-full py-3.5 bg-[#121212] hover:bg-black text-[#D4AF37] font-black text-xs rounded-xl shadow-md transition flex items-center justify-center gap-2 cursor-pointer disabled:opacity-60"
              >
                {isLoadingGoogle ? (
                  <>
                    <Loader2 className="w-4 h-4 animate-spin text-[#D4AF37]" />
                    <span>Sincronizando com Appwrite...</span>
                  </>
                ) : (
                  <>
                    <span>Entrar e Sincronizar Agora</span>
