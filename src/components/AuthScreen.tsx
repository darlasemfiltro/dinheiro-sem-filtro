import { cleanAndFormatRegistrationPayload } from '../utils/registration';
...
export const AuthScreen: React.FC<AuthScreenProps> = ({
  ...
  const sexoOptions: SearchableSelectOption[] = [
    { value: 'Masculino', label: 'Masculino' },
    { value: 'Feminino', label: 'Feminino' },
    { value: 'Outros', label: 'Outros / Prefiro não informar' },
  ];
  ...
  const isSexoValid = Boolean(sexo.trim());
  const isFormValid = isRegister
    ? (consentLgpd && isAgeValid && isLocationValid && isIncomeValid && isSexoValid && isNameValid && isEmailValid && isPasswordValid && password === signupConfirmPassword)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isRegister) {
      ...
      if (password !== signupConfirmPassword) {
        setError('As senhas não coincidem. A confirmação de senha está diferente da senha informada.');
        return;
      }
      if (!birthDate) {
    ...
    try {
      ...
      // 2. Registrar também no Appwrite se disponível e persistir documento na collection user_financials (não bloqueante)
        try {
          let accRes;
          try {
            accRes = await appwriteSignUp(cleanEmail, password, name);
          } catch (signUpErr: any) {
            if (signUpErr?.code === 409 || signUpErr?.type === 'user_already_exists') {
              console.log('[AuthScreen] Usuário já existe no Appwrite Auth, prosseguindo...');
              accRes = { email: cleanEmail };
            } else {
              console.warn('[AuthScreen] Appwrite signUp aviso (não bloqueante):', signUpErr);
              accRes = { email: cleanEmail };
            }
          }
          ...
          if (accRes) {
            try {
              const payload = cleanAndFormatRegistrationPayload({
                estado: state,
                cidade: city,
                dataNascimento: birthDate,
                rendaMensal: resolvedIncome,
                incomeBracket: incomeBracket.trim(),
                faixa_renda: incomeBracket.trim(),
                sexo: sexo.trim(),
                consentLgpd: consentLgpd,
                consent_lgpd: consentLgpd
              }, cleanEmail);

              await persistUserInitialDocument(accRes || { email: cleanEmail }, payload);
            } catch (dbErr) {
              console.warn('[AuthScreen] Appwrite database persist aviso (não bloqueante):', dbErr);
            }
          }
        } catch (appwriteErr: any) {
          console.warn('[Appwrite Signup/Persist Non-Blocking Warning]', appwriteErr);
        }
      } else {
  ...
  // Google Login Handler via Appwrite OAuth2
        ...
        {mode === 'auth' && (
          <div className="space-y-4">
            ...
            <form onSubmit={handleSubmit} className="space-y-3.5 sm:space-y-4" id="auth-form">
              ...
              {/* Confirmar Senha for Registration */}
              {isRegister && (
                <div className="space-y-1">
                  <label className="text-xs sm:text-sm font-bold text-[#121212]">Confirmar Senha</label>
                  <div className="relative">
                    <Lock className="w-4 h-4 text-[#D4AF37] absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
                    <input
                  ...
                  {signupConfirmPassword && password !== signupConfirmPassword && (
                    <p className="text-[10px] text-red-600 font-bold flex items-center gap-1 mt-1">
                      ⚠️ As senhas informadas estão diferentes.
                    </p>
                  )}
                </div>
              )}
              ...
              {isRegister && (
                <>
                  ...
                  <div className="space-y-1">
                    <label className="text-xs sm:text-sm font-bold text-[#121212]">
                      Sexo <span className="text-[#D4AF37]">*</span>
                    </label>
                    <div>
                      <SearchableSelect
                        options={sexoOptions}
                        ...
                        searchPlaceholder="Pesquisar sexo..."
                        emptyText="Nenhuma opção encontrada"
                        icon={<UserIcon className="w-4 h-4 text-[#D4AF37]" />}
                        id="signup-sexo"
                      />
                    </div>
                    <p className="text-[10px] text-gray-500 font-medium">
                      Informação utilizada para calibração de projeções financeiras demográficas.
                    </p>
                  </div>
