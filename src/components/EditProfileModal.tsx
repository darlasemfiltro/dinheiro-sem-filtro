...
interface EditProfileModalProps {
  ...
  onClose: () => void;
  user: User;
  onSaveName: (newName: string, avatarUrl?: string, city?: string, state?: string) => void;
...
export const EditProfileModal: React.FC<EditProfileModalProps> = ({
  ...
  const handleSaveProfileSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onSaveName(trimmedName, avatarUrl, cleanCity, cleanState);
      ...
      StorageService.updateUserProfile(user.email, trimmedName, avatarUrl);

      // Also persist to Appwrite database
      try {
        const canonicalId = user.email ? user.email.toLowerCase().trim() : user.id;
        await StorageService.syncUserDataWithAppwrite(canonicalId, {
        ...
        console.warn('[EditProfileModal appwrite sync notice]', e);
      }
