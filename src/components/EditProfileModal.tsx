...
export const EditProfileModal: React.FC<EditProfileModalProps> = ({
  ...
  // Pedir Acesso (Solicitar acesso por E-mail)
  const handleRequestAccessSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setRequestFeedback(null);
    if (!requestCodeOrEmail.trim()) {
      setRequestFeedback({ type: 'error', msg: 'Digite o E-mail do Titular do Orçamento.' });
      return;
    }

    const res = await StorageService.joinBudgetByCodeOrEmail(user, requestCodeOrEmail.trim());
    if (res.success) {
  ...
  const handleGiveAccessSubmit = async (e: React.FormEvent) => {
    ...
    const res = await StorageService.addCollaboratorByEmail(user, giveEmail.trim());
    if (res.success) {
      setGiveEmail('');
