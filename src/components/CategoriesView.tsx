...
const SubcategoryItem: React.FC<SubcategoryItemProps> = ({
  ...
  const isAddingChild = !isReadOnly && addingParentKey === currentKey;
...
export const CategoriesView: React.FC<CategoriesViewProps> = ({
  ...
  const handleAddSubcategory = (cat: Category, parentSubId: string | null, name: string) => {
    ...
    setNotification(`Subcategoria "${name.trim()}" cadastrada com sucesso!`);
  ...
  const handleRestoreDefaults = () => {
    ...
    window.dispatchEvent(new CustomEvent('app-confirm', {
      detail: {
        message: 'Deseja carregar a estrutura de Categorias padrão com base na Regra 50 / 30 / 20 e Receitas?',
        onConfirm: () => {
