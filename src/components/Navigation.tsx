  ...
  return (
    <>
      ...
      <nav className="bg-[#FAFAFA]/95 border-t border-gray-200 border-b border-[#D4AF37] backdrop-blur-md" id="main-navigation">
        {/* Dedicated Zoom Control Bar Above Menu */}
        <div className="bg-[#121212] border-b border-[#D4AF37]/50 px-3 py-1 flex items-center justify-between text-white text-xs shadow-inner" id="zoom-bar-above-menu">
          <div className="flex items-center gap-1.5 text-[#D4AF37] font-black text-[11px] tracking-wide">
            <ZoomIn className="w-3.5 h-3.5 text-[#D4AF37]" />
            <span>ZOOM DA TELA: <strong className="text-white">{zoomLevel}%</strong></span>
          </div>
          <div className="flex items-center gap-1.5">
            <button
              onClick={() => onZoomChange && onZoomChange(zoomLevel - 10)}
              disabled={zoomLevel <= 50}
              className="px-2 py-0.5 bg-gray-800 hover:bg-gray-700 text-white rounded-md font-bold disabled:opacity-30 cursor-pointer flex items-center gap-0.5 text-xs border border-gray-700"
              title="Diminuir Zoom (-10%)"
              id="above-menu-zoom-out"
            >
              <ZoomOut className="w-3 h-3 text-[#D4AF37]" />
              <span>-</span>
            </button>
            <button
              onClick={() => onZoomChange && onZoomChange(100)}
              className="px-2.5 py-0.5 bg-[#D4AF37] hover:bg-[#e6c252] text-[#121212] rounded-md font-black text-xs cursor-pointer flex items-center gap-1 shadow-xs"
              title="Redefinir Zoom para 100%"
              id="above-menu-zoom-reset"
            >
              <span>100%</span>
              {zoomLevel !== 100 && <RotateCcw className="w-2.5 h-2.5 text-[#121212]" />}
            </button>
            <button
              onClick={() => onZoomChange && onZoomChange(zoomLevel + 10)}
        ...
        <div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 py-1 sm:py-1.5 flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-1.5 min-w-0 shrink" id="navigation-left-section">
            <div
              className="min-h-[34px] sm:min-h-[42px] flex items-center gap-1 px-2.5 py-1 bg-[#D4AF37] border border-[#121212] rounded-lg sm:rounded-xl text-[#121212] font-black text-[10px] sm:text-xs truncate shadow-xs shrink-0"
              id="active-page-indicator"
            >
              <LayoutDashboard className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#121212] shrink-0 stroke-[3]" />
      ...
      {isOpen && createPortal(
        <div className="fixed inset-0 z-[999999] flex justify-end" id="right-navigation-drawer-overlay">
          ...
          >
            {/* Drawer Header */}
            <div className="flex items-center justify-between p-5 border-b border-[#D4AF37]/30 bg-[#121212] sticky top-0 z-20">
              <h2 className="text-[#D4AF37] text-sm font-black tracking-widest uppercase">MENU</h2>
