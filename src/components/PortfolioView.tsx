...
export const PortfolioView: React.FC<PortfolioViewProps> = ({
  ...
  const [targetAllocations, setTargetAllocations] = useState<TargetAllocation[]>(() =>
    PortfolioStorageService.getTargetAllocations(userId)
  );
  ...
  // Desempenho de patrimônio Chart Filters (Image 5)
  const [patCategoryFilter, setPatCategoryFilter] = useState<string>('completo');
    ...
    async function loadTransactions() {
      try {
        ...
        if (doc?.data) {
          ...
          if (Array.isArray(parsed.investmentTransactions)) {
            setTransactions(parsed.investmentTransactions);
            PortfolioStorageService.saveTransactions(parsed.investmentTransactions, userId);
          }
        }
      } catch (err) {
        console.error('Erro ao carregar investimentos da nuvem:', err);
      }
    }
    loadTransactions();
  }, []);
  ...
  useEffect(() => {
    ...
    // Set up automatic interval refresh for live updates across devices
    const interval = setInterval(async () => {
      await PortfolioStorageService.loadPortfolioFromRemote(userId);
      loadData();
      const { assets: newAssets, quotes: newQuotes } = await PortfolioStorageService.refreshMarketPrices(userId);
      setAssets(newAssets);
      setQuotes(newQuotes);
    }, 5000);
  ...
  const handleSaveAssetTransaction = async (tx: Omit<InvestmentTransaction, 'id' | 'createdAt'> | InvestmentTransaction) => {
    ...
    const txId = ('id' in tx && tx.id) ? tx.id : 'inv_' + Date.now();
    const formattedTx = {
      ...tx,
      id: txId,
      ticker: (tx as any).assetTicker || (tx as any).ticker || 'ATIVO',
      assetTicker: (tx as any).assetTicker || (tx as any).ticker || 'ATIVO',
      assetName: (tx as any).assetName || (tx as any).assetTicker || 'ATIVO',
      category: (tx as any).assetCategory || (tx as any).category || 'Ações',
      assetCategory: (tx as any).assetCategory || 'Ações',
      type: (tx as any).type || 'buy',
      price: (tx as any).unitPrice || (tx as any).price || 0,
      unitPrice: (tx as any).unitPrice || (tx as any).price || 0,
      quantity: (tx as any).quantity || 0,
      totalValue: ((tx as any).quantity || 0) * ((tx as any).unitPrice || (tx as any).price || 0),
      totalAmount: ((tx as any).quantity || 0) * ((tx as any).unitPrice || (tx as any).price || 0),
      institution: (tx as any).broker || (tx as any).institution || 'RICO INVESTIMENTOS',
      broker: (tx as any).broker || 'RICO INVESTIMENTOS',
    ...
    if ('id' in tx && tx.id) {
      PortfolioStorageService.updateTransaction(formattedTx as InvestmentTransaction, userId);
    } else {
      PortfolioStorageService.addTransaction(formattedTx as Omit<InvestmentTransaction, 'id' | 'createdAt'>, userId);
    }
    ...
    try {
      ...
      const updatedInvTxs = ('id' in tx && tx.id)
        ? existingInvTxs.map((t: any) => t.id === tx.id ? { ...t, ...formattedTx } : t)
        : [formattedTx, ...existingInvTxs];
  ...
  const segmentPieChartData = useMemo(() => {
    ...
    return uniqueSegments
      .map((s, idx) => {
        const pct = totalEquity > 0 ? (s.segVal / totalEquity) * 100 : 0;
        return {
          name: s.seg,
          segment: s.seg,
  ...
  const proventosByAsset = useMemo(() => {
    ...
    array.sort((a, b) => b.totalReceived - a.totalReceived);
    return array;
  }, [activeDividends, activeAssetsMap, assets, assetProventosFilter, assetProventosAppliedStartDate, assetProventosAppliedEndDate]);
  ...
  const calculatedTargetAllocations = useMemo(() => {
    ...
    return targetAllocations.map((item) => {
  ...
  const currentFilteredValue = useMemo(() => {
    if (patCategoryFilter === 'completo') {
      return patMetricType === 'patrimonio' ? totalEquity : totalInvested;
    }
    const cat = patCategoryFilter as AssetCategory;
  ...
  }, [patCategoryFilter, patMetricType, totalEquity, totalInvested, assets, quotes]);
  ...
  const activePatrimonioChartData = useMemo(() => {
    ...
    const catFilteredTxs = transactions.filter((t) => {
      if (patCategoryFilter === 'completo') return true;
      const tTicker = t.assetTicker || (t as any).ticker;
      const tAssetId = (t as any).assetId;
      const asset = assets.find((a) => a.id === tAssetId || a.ticker === tTicker);
      return asset ? asset.category === patCategoryFilter : true;
    });
  ...
  const handleSaveGoalForm = (e: React.FormEvent) => {
    ...
    if (editingGoal) {
      PortfolioStorageService.updateGoal(
        {
          ...editingGoal,
      ...
      PortfolioStorageService.addGoal(
        {
          userId,
          title: goalForm.title,
          targetAmount: parseFloat(goalForm.targetAmount),
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in duration-200">
      {/* Top Header Banner with Live Market Quotes */}
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-3">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black uppercase tracking-wider text-white font-serif">
                    Gráfico de Desempenho (Google Finance Style)
                  </span>
                </div>
                {/* Asset selector tabs */}
                          ...
                          if (active && payload && payload.length) {
                            ...
                            return (
                              <div className="bg-[#18181B] border-2 border-[#D4AF37] p-3 rounded-xl shadow-2xl text-xs space-y-1.5 z-50">
                                ...
                                <p className="text-gray-300">Período: <span className="text-gray-200 font-semibold">{periodLabel}</span></p>
                                <p className="text-gray-300">Valor: <span className="text-white font-mono font-bold">R$ {formatNumberToPtBr(data.price)}</span></p>
                                <p className="text-gray-300">Variação do Período: <span className={isPositive ? 'text-[#00E676] font-bold' : 'text-[#FF5252] font-bold'}>{isPositive ? '+' : ''}{metric.pct.toFixed(2)}%</span></p>
                              </div>
                            );
                          }
                          return null;
              ...
              >
                ...
                {(() => {
                  ...
                  for (let i = 0; i < pointsCount; i++) {
                    ...
                    const interpolated = startPrice + (currentPrice - startPrice) * progress + wave * amplitude;
                    data.push({ time: i, price: Number(interpolated.toFixed(2)) });
                  }
                  if (data.length > 0) {
                  ...
                  return (
                    <div className="h-16 w-full my-1.5 bg-[#121214]/70 rounded-xl overflow-hidden p-1 border border-white/5">
                      <ResponsiveContainer width="100%" height="100%">
                        <AreaChart data={data} margin={{ top: 4, right: 0, left: 0, bottom: 0 }}>
                          <defs>
                            <linearGradient id={`gradient-${q.id}`} x1="0" y1="0" x2="0" y2="1">
                              <stop offset="5%" stopColor={isPositive ? '#00E676' : '#FF5252'} stopOpacity={0.4}/>
                              <stop offset="95%" stopColor={isPositive ? '#00E676' : '#FF5252'} stopOpacity={0.0}/>
                            </linearGradient>
                          </defs>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                {calculatedTargetAllocations.map((item) => {
                  ...
                  return (
                    <div
                      key={item.id}
                      className={`p-3.5 rounded-2xl border transition space-y-2 relative ${
                        isOver
                    ...
                    >
                      <div className="flex items-center justify-between gap-1">
                        <span className="text-[11px] font-black uppercase font-serif text-white truncate max-w-[130px]">
                          {item.label}
                        </span>
                        <span
                      ...
                      <div className="flex items-baseline justify-between pt-1">
                        <div>
                          <span className="text-[9px] text-gray-400 block font-bold uppercase">Atual</span>
                          <span className="text-base font-black font-serif text-white">
                            {item.currentPct.toFixed(1)}%
                          </span>
                        </div>
                        <div className="text-right">
                          <span className="text-[9px] text-gray-400 block font-bold uppercase">Desejado</span>
                          <span className="text-sm font-black font-serif text-gray-300">
                            {item.targetPct}%
                          </span>
                        </div>
                      </div>
        ...
        <div className="space-y-6 animate-in fade-in">
          ...
          <div className="bg-[#18181B] border border-white/10 rounded-2xl p-5 sm:p-6 shadow-xl space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-white/10">
              ...
              {/* Top Right Dropdowns / Filters */}
              <div className="flex flex-wrap items-center gap-2">
                <select
                  value={patCategoryFilter}
                  onChange={(e) => setPatCategoryFilter(e.target.value)}
                  className="bg-[#121212] border border-white/20 text-white text-xs font-bold px-3 py-2 rounded-xl focus:outline-none focus:border-[#D4AF37] cursor-pointer"
                >
                  ...
                  <option value="etf_exterior">ETF Exterior</option>
                ...
                <select
                  value={patMetricType}
                  onChange={(e) => setPatMetricType(e.target.value as 'patrimonio' | 'investimento')}
                  className="bg-[#121212] border border-white/20 text-white text-xs font-bold px-3 py-2 rounded-xl focus:outline-none focus:border-[#D4AF37] cursor-pointer"
                >
                ...
                <select
                  value={patPeriodFilter}
                  onChange={(e) => setPatPeriodFilter(e.target.value as any)}
                  className="bg-[#121212] border border-white/20 text-white text-xs font-bold px-3 py-2 rounded-xl focus:outline-none focus:border-[#D4AF37] cursor-pointer"
                >
                  <option value="6M">6 Meses</option>
                  <option value="1Y">1 Ano (12M)</option>
                  <option value="2Y">2 Anos (24M)</option>
                ...
                <button
                  ...
                  className="bg-[#121212] border border-white/20 hover:border-[#D4AF37] text-white text-xs font-bold px-3 py-2 rounded-xl transition flex items-center gap-1.5 cursor-pointer"
            ...
            {patPeriodFilter === 'CUSTOM' && (
              <div className="bg-[#121212] border border-[#D4AF37]/30 rounded-xl p-3 flex flex-wrap items-center gap-4 text-xs">
                <div className="flex items-center gap-2">
                  <Calendar className="w-4 h-4 text-[#D4AF37] shrink-0" />
                  <span className="font-bold text-gray-300 shrink-0">Data Início:</span>
                ...
                <div className="flex items-center gap-2">
                  <span className="font-bold text-gray-300 shrink-0">Data Fim:</span>
                  <div className="flex items-center gap-1">
                    <input
                      type="text"
                      placeholder="DD/MM/AAAA"
                ...
                <span className="text-[11px] text-[#D4AF37] font-medium ml-auto">
                  Período ativado de {formatDateBRInput(patCustomStartDate) || 'Início'} até {formatDateBRInput(patCustomEndDate) || 'Hoje'}
                </span>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-[#121212] border border-white/10 rounded-xl space-y-1">
                <span className="text-xs font-bold text-gray-400">
                  ...
                  {patCategoryFilter === 'completo' ? 'Geral' : CATEGORY_LABELS[patCategoryFilter as AssetCategory] || patCategoryFilter})
                </span>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-black text-[#3B82F6] uppercase tracking-wider">
                    ...
                    {patCategoryFilter !== 'completo' && ` - ${CATEGORY_LABELS[patCategoryFilter as AssetCategory] || patCategoryFilter}`}
                  </span>
                </div>
          ...
          <div className="bg-[#18181B] border border-white/10 rounded-2xl p-5 sm:p-6 shadow-xl space-y-4">
            <div className="flex items-center justify-between border-b border-white/10 pb-3">
              ...
              <span className="text-xs font-black text-[#D4AF37]">
                Total: {formatValue((Object.keys(CATEGORY_LABELS) as AssetCategory[]).reduce((acc, c) => acc + getCategoryTotal(c), 0))}
              </span>
            </div>
                  ...
                  return (
                    <div key={cat} className="bg-[#121212] border border-white/10 rounded-2xl overflow-hidden shadow-lg">
                      <div
                        ...
                        className="p-4 bg-[#18181B] border-b border-white/10 flex items-center justify-between cursor-pointer hover:bg-white/5 transition"
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-gray-400">{catAssets.length} ativo(s) cadastrado(s)</p>
                          </div>
                        </div>

                        <div className="flex items-center gap-3">
                          <div className="text-right">
                            <p className="text-sm font-black text-white">{formatValue(catTotal)}</p>
                            ...
                            catAssets.map((asset) => {
                              ...
                              return (
                                <div
                                  ...
                                  className="p-3 bg-[#18181B] border border-white/5 rounded-xl flex items-center justify-between hover:border-[#D4AF37]/60 hover:bg-white/5 transition cursor-pointer group"
                                ...
                                >
                                  ...
                                  <div className="flex items-center gap-3">
                                    <div className="text-right">
                                      ...
                                      <div className="text-[10px] font-black flex items-center justify-end gap-1.5">
                                        <span className="text-gray-300">{assetPct.toFixed(1)}%</span>
        ...
        <div className="space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <input
                      ...
                      className="w-8 h-8 p-0 bg-[#18181B] border border-[#00C853]/60 rounded-xl text-white font-bold cursor-pointer focus:outline-none"
          ...
          <div className="bg-[#18181B] border border-white/10 rounded-2xl p-6 shadow-xl space-y-4">
            <div className="border-b border-white/10 pb-3 flex items-center justify-between">
              ...
              <span className="text-[10px] bg-[#00C853]/20 text-[#00E676] px-2.5 py-1 rounded-full font-black border border-[#00C853]/40">
                Sincronizado
              </span>
            </div>
            ...
            <div className="p-3 bg-[#121212] border border-white/10 rounded-xl flex items-center justify-between text-xs">
              <span className="font-bold text-gray-400">Média Mensal Estimada no Período:</span>
              <span className="font-black text-[#D4AF37] font-serif text-sm">
                  ...
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <input
                        ...
                        className="w-7 h-7 p-0 bg-[#18181B] border border-[#00C853]/60 rounded-lg text-white font-bold cursor-pointer"
                      />
                    </div>
                  </div>
        ...
        <div className="space-y-6">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <input
                            ...
                            className="w-8 h-8 p-0 bg-[#18181B] border border-[#D4AF37]/60 rounded-xl text-white font-bold cursor-pointer focus:outline-none"
                            title="Selecionar no calendário"
                          />
                        </div>
                      </div>
          ...
          <div className="p-6 bg-[#18181B] border border-white/10 rounded-2xl shadow-xl space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
              ...
              {/* Filtro de Ordenação de Rentabilidade */}
              <div className="flex items-center gap-2 bg-[#121212] p-1.5 rounded-xl border border-white/10">
                <span className="text-xs text-gray-400 font-bold pl-1">Filtrar Rentabilidade:</span>
                <select
                  value={rentabilitySortOrder}
                  onChange={(e) => setRentabilitySortOrder(e.target.value as 'DESC' | 'ASC')}
                  className="bg-[#18181B] border border-white/15 text-white rounded-lg px-2.5 py-1 text-xs font-bold focus:outline-none focus:border-[#D4AF37] cursor-pointer"
                >
              ...
              <table className="w-full text-left text-xs">
                <thead>
                  <tr className="border-b border-white/10 text-gray-400 font-black uppercase text-[10px]">
                    ...
                    <th className="py-3 px-2">Proventos</th>
                    <th className="py-3 px-2 text-right">
                      <button
                        onClick={() => setRentabilitySortOrder(rentabilitySortOrder === 'DESC' ? 'ASC' : 'DESC')}
                ...
                <tbody className="divide-y divide-white/5">
                  {assets.filter((a) => a.quantity > 0).length === 0 ? (
                    ...
                    assets
                      ...
                      .sort((a, b) => (rentabilitySortOrder === 'DESC' ? b.totalRent - a.totalRent : a.totalRent - b.totalRent))
                        ...
                        <table className="w-full text-left text-xs">
                          <thead>
                            <tr className="border-b border-white/10 text-gray-400 font-black uppercase text-[10px]">
                              ...
                              <th className="py-2.5 px-2">Preço Atual</th>
                              <th className="py-2.5 px-2">Quantidade</th>
                              <th className="py-2.5 px-2">Rent. Total</th>
                              <th className="py-2.5 px-2">Rent. Hoje</th>
                              <th className="py-2.5 px-2">% na Categoria</th>
                              <th className="py-2.5 px-2 text-right">% na Carteira</th>
                            </tr>
                          </thead>
                          <tbody className="divide-y divide-white/5">
                            {catAssets.map((a) => {
                          ...
                          <table className="w-full text-left text-xs">
                            <thead>
                              <tr className="border-b border-white/10 text-gray-400 font-black uppercase text-[10px]">
                                ...
                                <th className="py-2.5 px-2">Rent. Total</th>
                                <th className="py-2.5 px-2">Rent. Hoje</th>
                                <th className="py-2.5 px-2">% no Segmento</th>
                                <th className="py-2.5 px-2 text-right">% na Carteira</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-white/5">
        ...
        <div className="p-6 bg-[#18181B] border border-white/10 rounded-2xl shadow-xl space-y-4">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-[#121212] p-3 rounded-2xl border border-white/10">
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end">
              <button
                className="p-2 bg-[#18181B] border border-white/10 hover:border-[#00E676] text-gray-300 rounded-xl transition cursor-pointer"
      ...
      {isFilterModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/70 backdrop-blur-xs">
          <div className="w-full sm:w-[480px] bg-[#121212] border border-white/20 rounded-t-3xl sm:rounded-3xl text-white shadow-2xl p-6 space-y-6 animate-in slide-in-from-bottom duration-200">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <input
                          ...
                          className="w-8 h-8 p-0 bg-[#121212] border border-white/20 rounded-xl text-white font-bold cursor-pointer focus:outline-none focus:border-[#00C853]"
                          title="Selecionar no calendário"
                        />
                      </div>
                    </div>
            ...
            {/* Section: Categorias */}
            <div className="space-y-2">
              <label className="text-xs font-black text-gray-300 uppercase tracking-wider">Categorias</label>
              <select
                value={patCategoryFilter}
                onChange={(e) => {
                  setPatCategoryFilter(e.target.value);
                  setFilterCategory(e.target.value);
                }}
                className="w-full bg-[#18181B] border border-white/20 rounded-xl px-4 py-3 text-xs font-bold text-white focus:outline-none focus:border-[#00E676]"
              >
                <option value="completo">Completo (Todas as Categorias)</option>
                <option value="acoes">Ações</option>
                <option value="fiis">FIIs</option>
                <option value="fiagro">FIAGRO</option>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs">
          <div className="w-full max-w-md bg-[#121212] border border-[#D4AF37]/50 rounded-3xl text-white shadow-2xl p-6 space-y-4">
            ...
            <form onSubmit={handleSaveGoalForm} className="space-y-3 text-xs">
              ...
              {/* Asset Category Selection for Goals (Requirement: Image 14) */}
              <div>
                ...
                <select
                  ...
                  className="w-full bg-[#18181B] border border-white/20 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-[#D4AF37]"
                >
                  <option value="Patrimônio Total">Patrimônio Total</option>
                  <option value="Ações">Ações</option>
                  <option value="FIIs">FIIs</option>
                  <option value="Tesouro Direto">Tesouro Direto</option>
                  <option value="Criptomoedas">Criptomoedas</option>
                  <option value="Stocks">Stocks</option>
                  <option value="ETF Exterior">ETF Exterior</option>
                  <option value="FIAGRO">FIAGRO</option>
                </select>
              </div>
              ...
              <div className="p-4 bg-[#121212] border border-white/10 rounded-2xl space-y-4">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-3">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-[#00BFA5] hover:bg-[#00E676] text-[#121212] rounded-xl text-xs font-black transition flex items-center justify-center gap-1.5 cursor-pointer shadow-md"
                  >
                      ...
                      {assetTxs.map((t) => {
                        ...
                        return (
                          <tr key={t.id} className="hover:bg-white/5 transition font-bold">
                            ...
                            <td className="py-2.5 px-3 text-gray-400">Manual</td>
                            <td className="py-2.5 px-3 text-right">
                              <button className="p-1 text-gray-400 hover:text-white rounded transition" title="Opções">
                                <MoreHorizontal className="w-4 h-4" />
                              </button>
                            </td>
                          </tr>
