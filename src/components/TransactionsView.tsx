...
export const TransactionsView: React.FC<TransactionsViewProps> = ({
  ...
  const handleInitiateDelete = (tx: Transaction) => {
    const series = tx.parentInstallmentId
      ? transactions.filter((t) => t.parentInstallmentId === tx.parentInstallmentId)
      : [];
    if (series.length > 1) {
              ...
              {filteredTransactions.map((tx) => {
                ...
                return (
                  <tr key={tx.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="py-3 px-3 whitespace-nowrap">
                      {isReadOnly ? (
                        <span className="bg-gray-100 border border-gray-200 rounded-lg px-2.5 py-1 text-xs font-bold text-gray-700 select-none inline-block">
                          {isoToPtBr(tx.date) || tx.date}
                        </span>
                      ) : (
                        <input
                          type="date"
                          ...
                          className="border border-gray-300 rounded-lg px-2 py-1 text-xs font-bold text-[#121212] focus:outline-none focus:ring-1 focus:ring-[#D4AF37] transition bg-gray-50 hover:bg-gray-100 cursor-pointer"
                        />
                      )}
                    </td>
              ...
              <div key={tx.id} className="p-3.5 space-y-2 hover:bg-gray-50 transition">
                ...
                <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] pt-1">
                  <div className="flex items-center gap-2">
                    {/* Date */}
                    {isReadOnly ? (
                      <span className="bg-gray-100 border border-gray-200 rounded-md px-2 py-0.5 text-[11px] font-bold text-gray-700 select-none">
                        {isoToPtBr(tx.date) || tx.date}
                      </span>
                    ) : (
                      <input
                        ...
                        className="bg-gray-50 border border-gray-300 rounded-md px-1.5 py-0.5 text-[11px] font-bold text-[#121212] cursor-pointer"
                      />
                    )}
                    {/* Conta */}
                    {isTransfer ? (
