...
export const AddAssetModal: React.FC<AddAssetModalProps> = ({
  ...
  const formatQuantityInput = (val: number | string, category: AssetCategory) => {
  ...
  useEffect(() => {
    if (isOpen) {
      if (editingTransaction) {
        ...
        setQuantity(formatQuantityInput(editingTransaction.quantity, editingTransaction.assetCategory || 'acoes'));
  ...
  const currencySymbol = useMemo(() => {
    if (assetCategory === 'cripto') return 'USDT';
  ...
  const handleSave = (keepOpen: boolean): boolean => {
    ...
    const payload = {
      ...(editingTransaction ? { id: editingTransaction.id, createdAt: editingTransaction.createdAt } : { id: 'tx_' + Date.now(), createdAt: new Date().toISOString() }),
      userId,
    ...
    alert(`[DEDO-DURO] Salvando Ativo:\nTicker: ${payload.assetTicker}\nTotal: R$ ${payload.totalAmount}`);
      ...
      <div className="bg-[#18181B] text-white border-2 border-[#00C853]/60 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 overflow-y-auto flex-1 text-sm bg-[#18181B]">
          ...
          <div ref={categoryContainerRef} className="relative">
            ...
            {isCategoryOpen && (
              <div className="absolute z-30 top-full left-0 right-0 mt-1 bg-[#121212] border-2 border-[#00C853]/80 rounded-xl shadow-2xl max-h-60 overflow-hidden flex flex-col animate-in fade-in duration-150">
                <div className="p-2 border-b border-white/10 bg-[#121212]">
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-[#18181B] border border-white/20 rounded-lg pl-9 pr-3 py-2 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-[#00C853]"
                      autoFocus
                    />
                  </div>
                </div>
                <div className="overflow-y-auto divide-y divide-white/5 flex-1 max-h-48">
                  {(Object.keys(CATEGORY_LABELS) as AssetCategory[])
                    .filter((cat) => CATEGORY_LABELS[cat].toLowerCase().includes(categorySearch.toLowerCase()))
                    .map((cat) => (
                      <button
                        key={cat}
                        type="button"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  onBlur={() => {
                    if (quantity.trim()) {
                      setQuantity(formatQuantityInput(quantity, assetCategory));
                    }
                  }}
                  placeholder="0"
                  required
