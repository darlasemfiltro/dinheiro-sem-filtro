...
export const Header: React.FC<HeaderProps> = ({
  ...
  const hasPending = StorageService.hasPendingSync(effectiveBudgetId);
  const [syncStatus, setSyncStatus] = useState<'syncing' | 'synced' | 'error'>('synced');
  const activeSyncStatus = propSyncStatus !== undefined ? propSyncStatus : (hasPending ? 'syncing' : syncStatus);
  const [imageError, setImageError] = useState(false);
  const userInitial = user.name ? user.name.trim().charAt(0).toUpperCase() : 'U';
  const myEmail = (user.email || '').trim().toLowerCase();
  const myId = user.id;
  const sharedBudget = StorageService.getSharedBudget(effectiveBudgetId, user);
  const personalBudgetId = StorageService.getCanonicalUserId(user.id || user.email || 'default');
  const isOwnBudget = effectiveBudgetId === personalBudgetId;

  const isReadOnly = StorageService.isCurrentUserReadOnly(user);
  const isOwner = StorageService.isBudgetOwner(user, effectiveBudgetId);

  const badgeText = isOwner 
    ? '👑 TITULAR DO ORÇAMENTO' 
    : (isReadOnly ? '📖 MEMBRO (LEITURA)' : '✏️ MEMBRO (EDIÇÃO)');
  const badgeStyle = isOwner 
    ? 'bg-[#00C853] text-[#121212]' 
    : 'bg-amber-500 text-white border border-amber-600';

  const [, forceUpdate] = useState({});
  useEffect(() => {
    const handleUpdate = () => {
      setPendingNotifications(StorageService.getPendingNotifications(user.email));
      forceUpdate({});
    };
    window.addEventListener('shared_budget_updated', handleUpdate);
    window.addEventListener('shared_budgets_updated', handleUpdate);
    window.addEventListener('financial_data_mutated', handleUpdate);
    window.addEventListener('remote_data_updated', handleUpdate);
    return () => {
      ...
      window.removeEventListener('financial_data_mutated', handleUpdate);
      window.removeEventListener('remote_data_updated', handleUpdate);
    };
  }, [user.email]);
  ...
  useEffect(() => {
    ...
    window.addEventListener('sync-status-change', handleStatus);
    return () => window.removeEventListener('sync-status-change', handleStatus);
  }, []);
    ...
    >
          ...
          <div className="relative shrink-0 ml-auto flex items-center gap-1.5 sm:gap-2.5" id="header-profile-dropdown">
            <button
              onClick={async () => {
                ...
                window.dispatchEvent(new CustomEvent('sync-status-change', { detail: 'syncing' }));
                try {
                  const effectiveBudgetId = StorageService.getEffectiveBudgetId(user);
                  const currentState = {
                  ...
                  const ok = await persistCurrentStateToAppwrite(effectiveBudgetId, currentState);
                  if (ok) {
                    StorageService.clearPendingSync(effectiveBudgetId);
                    setSyncStatus('synced');
                    window.dispatchEvent(new CustomEvent('sync-status-change', { detail: 'synced' }));
                    window.dispatchEvent(new CustomEvent('app-toast', { detail: 'Sincronizado com sucesso na nuvem!' }));
                  } else {
                    setSyncStatus('error');
                    window.dispatchEvent(new CustomEvent('sync-status-change', { detail: 'error' }));
                    window.dispatchEvent(new CustomEvent('app-toast', { detail: 'Falha ao sincronizar com a nuvem.' }));
                  }
                } catch (err: any) {
                  setSyncStatus('error');
                  window.dispatchEvent(new CustomEvent('sync-status-change', { detail: 'error' }));
                  window.dispatchEvent(new CustomEvent('app-toast', { detail: 'Erro de sincronização: ' + (err.message || 'Erro desconhecido') }));
                }
              }}
            ...
            {showUserMenu && createPortal(
              <div className="fixed inset-0 z-[999999] flex justify-end" id="header-user-drawer-overlay">
                ...
                >
                  {/* Drawer Header */}
                  <div className="p-3.5 sm:p-5 border-b border-gray-200 flex items-center justify-between gap-2 bg-gradient-to-r from-amber-50 to-white">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <div className="w-12 h-12 sm:w-16 sm:h-16 rounded-full bg-[#121212] text-[#D4AF37] font-black text-lg sm:text-2xl flex items-center justify-center uppercase shrink-0 shadow-lg ring-2 ring-[#D4AF37] overflow-hidden border-2 border-[#D4AF37]">
                        {user.avatarUrl && !imageError ? (
                          <img
                            src={user.avatarUrl}
                            alt="Avatar do Usuário"
                      ...
                      <div className="min-w-0 flex-1">
                        <h2 className="text-xs sm:text-sm font-black text-[#121212] flex items-center gap-1 truncate">
                          <span className="truncate">{user.name}</span>
                          <Shield className="w-3.5 h-3.5 text-[#D4AF37] shrink-0" />
                        </h2>
                        <p className="text-[11px] text-gray-500 truncate">{user.email}</p>
                        <span className={`inline-block mt-1 px-2 py-0.5 text-[9px] sm:text-[10px] font-black rounded-md uppercase tracking-wider ${badgeStyle}`}>
                          {badgeText}
                        </span>
                      </div>
                    </div>
                    ...
                    <button
                      onClick={() => setShowUserMenu(false)}
                      className="p-1.5 rounded-xl bg-gray-100 hover:bg-[#D4AF37] text-gray-700 hover:text-[#121212] transition cursor-pointer shrink-0"
                  ...
                  <div className="p-4 space-y-2 flex-1">
                    ...
                    <p className="text-[10px] uppercase font-black tracking-widest text-[#D4AF37] px-1 py-1">
                      {isOwner ? 'OPÇÕES DO TITULAR:' : 'OPÇÕES DO MEMBRO:'}
                    </p>
                    ...
                    <button
                      ...
                      className="w-full text-left p-3 bg-red-50 hover:bg-red-100 text-red-600 hover:border-red-400 border border-red-200 rounded-2xl text-xs font-bold transition flex items-center gap-3 cursor-pointer"
                    ...
                    <div className="p-3 bg-gradient-to-br from-amber-50/80 to-gray-50 border border-amber-200 rounded-2xl space-y-2 shadow-2xs">
                      <div className="flex items-center justify-between text-xs font-black text-[#121212]">
                        <span className="flex items-center gap-1.5 truncate">
                          <ZoomIn className="w-4 h-4 text-[#D4AF37] shrink-0" />
                          <span className="truncate">Zoom do Aplicativo ({zoomLevel}%)</span>
                        </span>
                        {zoomLevel !== 100 && (
                          <button
                            onClick={() => onZoomChange && onZoomChange(100)}
                            className="text-[10px] font-black text-[#D4AF37] hover:underline flex items-center gap-0.5 cursor-pointer shrink-0 ml-1"
                      ...
                      <div className="flex items-center justify-between gap-1.5">
                        <button
                          ...
                          className="px-2.5 py-1.5 bg-white hover:bg-amber-100 text-gray-800 border border-gray-300 rounded-xl text-xs font-black flex items-center justify-center gap-1 cursor-pointer disabled:opacity-40 shadow-2xs shrink-0"
                          title="Diminuir Zoom"
                        >
                          <ZoomOut className="w-3.5 h-3.5" />
                          <span>-10%</span>
                        </button>
                        <div className="flex-1 py-1.5 bg-gray-200/80 rounded-xl text-xs font-black text-[#121212] text-center border border-gray-300">
                          {zoomLevel}%
                        </div>
                        <button
                          onClick={() => onZoomChange && onZoomChange(zoomLevel + 10)}
                    ...
                    {isOwner ? (
                      ...
                      <button
                        ...
                        className="w-full text-left p-3.5 bg-gray-50 hover:bg-gray-100 text-gray-500 border border-gray-200 rounded-2xl text-xs font-bold transition flex items-center justify-between gap-3 cursor-pointer group"
                        id="user-drawer-critical-actions-disabled-btn"
                        title="Ações críticas ativas exclusivamente para o titular da conta"
                      >
                        <div className="flex items-center gap-3">
                          <ShieldAlert className="w-4 h-4 text-gray-400 shrink-0" />
                          <span>Menu de Ações Críticas</span>
                        </div>
                        <span className="text-[9px] bg-gray-200 text-gray-600 px-2 py-0.5 rounded-md font-bold uppercase tracking-wider whitespace-nowrap shrink-0 flex items-center gap-1">
                          APENAS TITULAR
                        </span>
                      </button>
                    )}
      ...
      {syncToast && (
        <div className="fixed bottom-5 right-5 z-[999999] bg-[#121212] text-white border-2 border-[#D4AF37] px-4 py-3 rounded-2xl shadow-2xl flex items-center gap-3 animate-in fade-in slide-in-from-bottom-3 duration-200">
          <Wifi className="w-5 h-5 text-[#D4AF37] shrink-0 animate-bounce" />
