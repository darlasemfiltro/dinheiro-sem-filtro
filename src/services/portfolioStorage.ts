...
export const SEED_ASSETS: InvestmentAsset[] = [
  ...
  // CRIPTO
  {
    ...
    quantity: 0.0031,
    averagePrice: 339000.0,
    currentPrice: 330913.31,
    priceChange24h: 0.50,
    priceChange24hValue: 1654.56,
    currency: 'BRL',
    dy: 0,
    yieldOnCost: 0,
    lastDividendValue: 0,
  ...
  },
  {
    ...
    quantity: 0.035,
    averagePrice: 19800.0,
    currentPrice: 18500.0,
    priceChange24h: -0.86,
    priceChange24hValue: -5.58,
    currency: 'BRL',
    dy: 0,
    yieldOnCost: 0,
    lastDividendValue: 0,
...
export const SEED_TRANSACTIONS: InvestmentTransaction[] = [
  ...
  { id: 'tx_inv_6', userId: 'default', assetTicker: 'BTC', assetCategory: 'cripto', type: 'buy', quantity: 0.0031, unitPrice: 339000.0, totalAmount: 1050.9, broker: 'BINANCE', date: '2026-06-02', notes: 'Sats stacking', createdAt: new Date().toISOString() },
  { id: 'tx_inv_7', userId: 'default', assetTicker: 'ETH', assetCategory: 'cripto', type: 'buy', quantity: 0.035, unitPrice: 19800.0, totalAmount: 693.0, broker: 'BINANCE', date: '2026-06-02', notes: 'DCA Crypto', createdAt: new Date().toISOString() },
  ...
  { id: 'tx_inv_pendle', userId: 'default', assetTicker: 'PENDLE', assetCategory: 'cripto', type: 'buy', quantity: 12, unitPrice: 28.00, totalAmount: 336.00, broker: 'BINANCE', date: '2026-06-02', notes: 'DCA Crypto', createdAt: new Date().toISOString() },
];
...
export class PortfolioStorageService {
  ...
  static getAssets(userId = 'default'): InvestmentAsset[] {
    try {
      ...
      const activeOnly = parsed.filter((a) => a.quantity > 0);
  ...
  static syncAssetForTicker(ticker: string, category: AssetCategory, userId = 'default') {
    ...
    if (totalQty > 0) {
      if (existingIndex >= 0) {
        assets[existingIndex].quantity = Number(Number(totalQty).toFixed(2));
  ...
  static async getHistoricalQuote(ticker: string, dateStr: string): Promise<number | null> {
    ...
    let basePrice = matchingMarketQuote?.price || existing?.currentPrice || basePrices[cleanTicker];
