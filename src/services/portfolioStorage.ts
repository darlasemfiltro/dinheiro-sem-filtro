...
import { getCanonicalUserId, resolveBudgetId, StorageService } from './storage';
import {
...
export class PortfolioStorageService {
  ...
  static async loadPortfolioFromRemote(userId = 'default', skipPush = false): Promise<void> {
    try {
      ...
      const assets = rawAssets.filter((a: any) => {
        ...
        if (key && (deletedIds.has(key) || deletedIds.has(String(key).toUpperCase()))) return false;
        return true;
      });
      ...
      const transactions = rawTransactions.filter((t: any) => {
        ...
        if (deletedIds.has(t.id) || deletedIds.has(t.id.toUpperCase())) return false;
        return true;
      });
      ...
      const dividends = rawDividends.filter((d: any) => {
        if (!d || !d.id) return false;
        if (deletedIds.has(d.id) || deletedIds.has(d.id.toUpperCase())) return false;
        return true;
      });
  ...
  static deleteTransaction(id: string, userId = 'default') {
    ...
    try {
      localStorage.setItem('dsf_investments_cache', JSON.stringify(filtered));
      localStorage.setItem(`dsf_investments_cache_${userId}`, JSON.stringify(filtered));
    } catch {}
