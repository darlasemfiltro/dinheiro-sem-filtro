...
export class StorageService {
  ...
  static setAccounts(accounts: Account[], budgetId?: string) {
    this.initialize();
    const canonicalId = getCanonicalUserId(budgetId || accounts[0]?.userId || 'default');
    const accountsKey = `darla_accounts_${canonicalId}`;
  ...
  static setCategories(categories: Category[], budgetId?: string) {
    this.initialize();
    const canonicalId = getCanonicalUserId(budgetId || categories[0]?.userId || 'default');
    const key = `darla_categories_${canonicalId}`;
  ...
  static setFamilyMembers(familyMembers: FamilyMember[], budgetId?: string) {
    this.initialize();
    const canonicalId = getCanonicalUserId(budgetId || familyMembers[0]?.userId || 'default');
  ...
  static setTransactions(transactions: Transaction[], budgetId?: string) {
    this.initialize();
    const canonicalId = getCanonicalUserId(budgetId || transactions[0]?.userId || 'default');
  ...
  static setGoals(goals: Goal[], budgetId?: string) {
    this.initialize();
    const canonicalId = getCanonicalUserId(budgetId || goals[0]?.userId || 'default');
  ...
  static getTransactions(budgetId: string): Transaction[] {
    ...
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw);
        if (Array.isArray(parsed) && parsed.length >= 0) {
          return parsed.map((t: any) => ({ ...t, userId: canonicalId }));
        }
      }
    } catch (e) {}
    ...
    if (fallback.length > 0) {
      return fallback.map((t: any) => ({ ...t, userId: canonicalId }));
    }
