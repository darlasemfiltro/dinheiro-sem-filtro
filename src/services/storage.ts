...
export class StorageService {
  ...
  static getCurrentUser(): User | null {
    ...
    try {
      ...
      const savedActive = localStorage.getItem('darla_active_budget_id');
      if (savedActive && (!user.budgetId || user.budgetId !== savedActive)) {
        user.budgetId = savedActive;
        localStorage.setItem(STORAGE_KEYS.CURRENT_USER, JSON.stringify(user));
      }
  ...
  static updateUserProfile(userId: string, newName: string, avatarUrl?: string): User | null {
    ...
    const currentActiveBudget = (() => {
      try {
        const cur = JSON.parse(localStorage.getItem(STORAGE_KEYS.CURRENT_USER) || '{}');
        return cur.budgetId || localStorage.getItem('dsf_current_active_budget') || localStorage.getItem('darla_active_budget_id') || null;
      } catch (e) {
        return localStorage.getItem('dsf_current_active_budget') || localStorage.getItem('darla_active_budget_id') || null;
      }
    })();
    ...
    try {
      ...
      if (idx !== -1) {
        ...
        if (currentActiveBudget) {
          users[idx].budgetId = currentActiveBudget;
        }
        localStorage.setItem(STORAGE_KEYS.USERS, JSON.stringify(users));
      }
    } catch (e) {
    ...
    if (currentUser) {
      ...
      if (currentActiveBudget) {
        currentUser.budgetId = currentActiveBudget;
      }
  ...
  static getEffectiveBudgetId(user: User): string {
    ...
    const savedActive = localStorage.getItem('dsf_current_active_budget') || localStorage.getItem('darla_active_budget_id');
    if (savedActive) {
      const myId = getCanonicalUserId(user.id);
      const myEmail = (user.email || '').trim().toLowerCase();
      const canonicalSaved = getCanonicalUserId(savedActive);
      if (canonicalSaved === myId || canonicalSaved === myEmail || savedActive === user.id || savedActive === user.email || savedActive === user.budgetId) {
        return getCanonicalUserId(savedActive);
      }
  ...
  static switchBudget(currentUser: User, newBudgetId?: string): User {
    this.initialize();
    const targetBudgetId = newBudgetId || currentUser.id;
    localStorage.setItem('dsf_current_active_budget', targetBudgetId);
    localStorage.setItem('darla_active_budget_id', targetBudgetId);
  ...
  static getAccounts(budgetId: string): Account[] {
    ...
    // If not found in specific key, return empty array to prevent cross-contamination
    return [];
  }
