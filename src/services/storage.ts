console.log('[DEBUG] storage.ts carregado com sucesso v2');
...
import { getAppwriteUser, appwriteDatabases, getAppwriteConfig, appwritePasswordReset, account as appwriteAccount } from '../lib/appwrite';
...
export function resolveBudgetId(providedId?: string): string {
  if (providedId && providedId !== 'default' && providedId.trim() !== '') {
    return getCanonicalUserId(providedId);
  }
  if (typeof window !== 'undefined') {
    const active = localStorage.getItem('dsf_current_active_budget') || localStorage.getItem('darla_active_budget_id');
    if (active && active !== 'default' && active.trim() !== '') {
      return getCanonicalUserId(active);
    }
  }
  try {
    const curStr = localStorage.getItem(STORAGE_KEYS.CURRENT_USER);
    if (curStr) {
      const cur = JSON.parse(curStr);
      if (cur?.id || cur?.email || cur?.budgetId) {
        return getCanonicalUserId(cur.budgetId || cur.id || cur.email);
      }
    }
  } catch (e) {}
  try {
    const currUser = StorageService.getCurrentUser();
    if (currUser && (currUser.id || currUser.email || currUser.budgetId)) {
      return getCanonicalUserId(currUser.budgetId || currUser.id || currUser.email);
    }
  } catch (e) {}
...
export class StorageService {
  static getCanonicalUserId(userId?: string): string {
    return getCanonicalUserId(userId);
  }

  static isCloudSynced: boolean = false;
  private static isSyncingSharedBudgets: boolean = false;
  ...
  static purgeDeletedItems(canonicalId: string) {
    if (!canonicalId) return;
    const deletedIds = this.getDeletedIds(canonicalId);
    if (deletedIds.size === 0) return;
    _inMemoryStore.accounts = _inMemoryStore.accounts.filter(a => !deletedIds.has(a.id));
    _inMemoryStore.categories = _inMemoryStore.categories.filter(c => !deletedIds.has(c.id));
    _inMemoryStore.familyMembers = _inMemoryStore.familyMembers.filter(f => !deletedIds.has(f.id));
    _inMemoryStore.transactions = _inMemoryStore.transactions.filter(t => !deletedIds.has(t.id));
    _inMemoryStore.goals = _inMemoryStore.goals.filter(g => !deletedIds.has(g.id));
  }
  ...
  static async syncUserMutationToServer(userId: string): Promise<boolean> {
    ...
    try {
      ...
      try {
        ...
        const item = allFin[canonicalId] || allFin[Object.keys(allFin)[0]];
        if (item && item.data) {
          const parsed = typeof item.data === 'string' ? JSON.parse(item.data) : item.data;
          member_permissions = parsed.member_permissions || {};
        }
      } catch(e) {}
  ...
  static setCurrentUser(user: User | null) {
    ...
    if (!user) {
      ...
      try {
        ...
        const idx = users.findIndex(u => (u.email && user.email && u.email.toLowerCase() === user.email.toLowerCase()) || u.id === user.id);
        if (idx >= 0) {
          users[idx] = user;
        } else {
          users.push(user);
  ...
  static getCurrentUser(): User | null {
    ...
    try {
      ...
      const personalBudgetId = getCanonicalUserId(user.id || user.email || 'default');
      if (!user.budgetId || user.budgetId !== personalBudgetId) {
        user.budgetId = personalBudgetId;
        localStorage.setItem(STORAGE_KEYS.CURRENT_USER, JSON.stringify(user));
      }
      const canonicalId = getUserIdForEmail(user.email);
      // By default, avatarUrl should be empty so that the first initial of the name appears by default
      if (user.avatarUrl && (user.avatarUrl.includes('googleusercontent.com') || user.avatarUrl.includes('default-user') || user.avatarUrl.includes('logo') || user.avatarUrl.includes('darla_logo') || user.avatarUrl.includes('lotus'))) {
        user.avatarUrl = '';
        localStorage.setItem(STORAGE_KEYS.CURRENT_USER, JSON.stringify(user));
      }
  ...
  static async isUserRegisteredAsync(email: string): Promise<{ exists: boolean; user: User | null }> {
    ...
    if (isDarlaAccount(cleanEmail)) {
      const darlaUser: User = {
        id: cleanEmail,
        name: cleanEmail.includes('darlla') ? 'Darlla Carvalho' : 'Darla Sem Filtro',
        email: cleanEmail,
        isPro: true,
        plan: 'lifetime',
        subscriptionStatus: 'active',
  ...
  static async ensureUserAndDataSyncedAsync(
    ...
    const activeBudgetStored = localStorage.getItem('dsf_current_active_budget') || localStorage.getItem('darla_active_budget_id');
    const existingBudgetId = activeBudgetStored || existingUser?.budgetId || _inMemoryStore.currentUser?.budgetId || (() => {
      try {
    ...
    const userToSave: User = {
      ...
      authProvider: authProvider || existingUser?.authProvider || 'email',
      avatarUrl: avatarUrl || (existingUser?.avatarUrl && !existingUser.avatarUrl.includes('logo') && !existingUser.avatarUrl.includes('lotus') ? existingUser.avatarUrl : ''),
      createdAt: existingUser?.createdAt || new Date().toISOString(),
      isPro: isDarla ? true : (existingUser?.isPro ?? false),
    ...
    // Ensure local accounts and categories exist for this user immediately
    const userAccs = _inMemoryStore.accounts.filter(a => getCanonicalUserId(a.userId) === deterministicId);
    const userCats = _inMemoryStore.categories.filter(c => getCanonicalUserId(c.userId) === deterministicId);
    ...
    if (userAccs.length === 0 && userCats.length === 0) {
      ...
      _inMemoryStore.categories = _inMemoryStore.categories.concat(seededCats);
      _inMemoryStore.accounts = _inMemoryStore.accounts.concat(seededAccs);
      try {
    ...
    // Trigger background sync without blocking return
    (async () => {
      try {
        await checkAndMigrateOrFetchUserFinancials(
          deterministicId,
          () => ({
  ...
  static async sendPasswordResetCodeAsync(email: string): Promise<{ success: boolean; message: string; code?: string; emailSent?: boolean }> {
    ...
    // Trigger Appwrite Cloud Account Recovery email
    try {
      await appwritePasswordReset(cleanEmail);
    } catch (e) {
      console.warn('[Appwrite Password Reset recovery notice]', e);
    }
  ...
  static updateUserProfile(userId: string, newName: string, avatarUrl?: string, city?: string, state?: string, incomeBracket?: string, monthlyIncome?: number): User | null {
    this.initialize();
    ...
    try {
      ...
      if (idx !== -1) {
        ...
        if (city !== undefined) users[idx].city = city;
        if (state !== undefined) users[idx].state = state;
        if (incomeBracket !== undefined) users[idx].incomeBracket = incomeBracket;
        if (monthlyIncome !== undefined) users[idx].monthlyIncome = monthlyIncome;
        _inMemoryStore.users = users;
        localStorage.setItem(STORAGE_KEYS.USERS, JSON.stringify(users));
      }
    } catch (e) {}
    ...
    if (currentUser) {
      ...
      if (incomeBracket !== undefined) currentUser.incomeBracket = incomeBracket;
  ...
  static async resetUserBudgetToZero(budgetId: string, requestingUser?: User): Promise<void> {
    this.initialize();

    // Validação estrita de titularidade
    const u = requestingUser || this.getCurrentUser();
    if (u && !this.isBudgetOwner(u, budgetId)) {
      console.warn('[SEGURANÇA] Bloqueio: Usuário não é o titular do orçamento', { user: u.email, budgetId });
      throw new Error('Ação bloqueada: Apenas o titular do orçamento possui permissão para zerar os lançamentos.');
    }
    ...
    // 2. Update (Upsert) in Appwrite Database user_financials without deleting the document record
    ...
    if (config.projectId && config.projectId !== 'default-placeholder') {
      ...
      const currentUserObj = requestingUser || this.getCurrentUser() || (() => {
        try {
          return JSON.parse(localStorage.getItem('darla_current_user') || localStorage.getItem('currentUser') || '{}');
        } catch { return {}; }
      })();

      const cidadeVal = String(currentUserObj?.city || 'Brasília').trim().slice(0, 50);
      const estadoVal = String(currentUserObj?.state || 'DF').trim().toUpperCase().slice(0, 2);
      const birthVal = currentUserObj?.birthDate || '1995-01-27T12:00:00.000Z';
      const faixaVal = String(currentUserObj?.incomeBracket || currentUserObj?.faixa_renda || '').trim();
      const sexoVal = String(currentUserObj?.sexo || currentUserObj?.gender || 'Outros').trim();
      const consentVal = currentUserObj?.consent_lgpd ?? true;
      ...
      const fullResetPayload = {
        ...
        cidade: cidadeVal,
        estado: estadoVal,
        data_nascimento: birthVal,
        faixa_renda: faixaVal,
        sexo: sexoVal,
        consent_lgpd: consentVal
      };

      try {
        let existingDocId = targetDocId;
        try {
          const checkDoc = await appwriteDatabases.getDocument(config.databaseId, 'user_financials', targetDocId);
          if (checkDoc && checkDoc.$id) {
            existingDocId = checkDoc.$id;
          }
        } catch {
          try {
            ...
            if (list.documents.length > 0) {
              existingDocId = list.documents[0].$id;
            }
          } catch {}
        }
        ...
        try {
          ...
          console.log('[DEDO-DURO SUCESSO] Documento atualizado (zerado) no Appwrite:', existingDocId);
        } catch (updateErr) {
          try {
            ...
            console.log('[DEDO-DURO SUCESSO] Documento criado (zerado) no Appwrite:', targetDocId);
          } catch (createErr) {
            await appwriteDatabases.createDocument(
              config.databaseId,
              'user_financials',
        ...
        console.warn('[DEDO-DURO] Erro ao atualizar orçamento zerado no Appwrite:', err);
      }
    ...
    fetch('/api/data/reset', {
      ...
      body: JSON.stringify({ 
        userId: budgetId,
        requestingUserId: u ? (u.id || u.email) : budgetId
      }),
    }).catch(() => {});
  }
  ...
  static getEffectiveBudgetId(user: User): string {
    ...
    const userKey = (user.email || user.id || 'default').toLowerCase();
    const personalBudgetId = getCanonicalUserId(user.email || user.id || 'default');
    
    const savedActive = localStorage.getItem(`dsf_active_budget_${userKey}`) || personalBudgetId;
    const isViewingShared = localStorage.getItem(`dsf_viewing_shared_budget_${userKey}`) === 'true';

    if (!isViewingShared || savedActive === personalBudgetId || getCanonicalUserId(savedActive) === personalBudgetId) {
      return personalBudgetId;
    }

    const canonicalSaved = getCanonicalUserId(savedActive);
    if (canonicalSaved === personalBudgetId || savedActive === user.id || savedActive === user.email) {
      return personalBudgetId;
    }

    const allBudgets = this.deduplicateSharedBudgets();
    const targetBudget = allBudgets.find(b => b.budgetId === savedActive || b.budgetId === canonicalSaved || b.ownerEmail?.toLowerCase() === savedActive.toLowerCase());
    if (targetBudget) {
      const myId = getCanonicalUserId(user.id);
      const myEmail = (user.email || '').trim().toLowerCase();
      const isOwner = targetBudget.ownerId === myId || targetBudget.ownerId === myEmail || targetBudget.ownerEmail?.toLowerCase() === myEmail;
      if (isOwner) return targetBudget.budgetId;
      const collab = targetBudget.collaborators?.find(c => c.email.toLowerCase() === myEmail);
      if (collab) {
        return targetBudget.budgetId;
      }
    }
    return personalBudgetId;
  }
  ...
  static async syncSharedBudgetsWithServer(userEmail: string): Promise<SharedBudget[]> {
    this.initialize();
    if (!userEmail) return [];
    if (this.isSyncingSharedBudgets) {
      return this.deduplicateSharedBudgets();
    }
    this.isSyncingSharedBudgets = true;
    try {
      ...
      if (res.ok) {
        ...
        for (const b of serverBudgets) {
          ...
          if (b.ownerEmail && b.ownerEmail.toLowerCase() !== cleanEmail) {
            try {
              const ownerRes = await fetch(`/api/data/load?userId=${encodeURIComponent(b.ownerEmail)}`);
              if (ownerRes.ok) {
                const ownerDataJson = await ownerRes.json();
                if (ownerDataJson.success && ownerDataJson.data) {
                  const allFinStr = localStorage.getItem('DINHEIRO_SEM_FILTRO_USER_FINANCIALS') || '{}';
                  const allFin = JSON.parse(allFinStr);
                  allFin[b.ownerEmail.toLowerCase()] = {
                    email: b.ownerEmail,
                    data: ownerDataJson.data,
                    updatedAt: ownerDataJson.updatedAt || new Date().toISOString()
                  };
                  localStorage.setItem('DINHEIRO_SEM_FILTRO_USER_FINANCIALS', JSON.stringify(allFin));

                  const parsedOwnerData = typeof ownerDataJson.data === 'string' ? JSON.parse(ownerDataJson.data) : ownerDataJson.data;
                  const perms = parsedOwnerData.member_permissions || {};
                  const myPerm = perms[cleanEmail];
                  if (myPerm) {
                    const expectedAccessMode = (String(myPerm).toLowerCase() === 'leitura' || String(myPerm).toLowerCase() === 'read') ? 'read' : 'edit';
                    const sharedBudget = map.get(key);
                    if (sharedBudget && Array.isArray(sharedBudget.collaborators)) {
                      const collab = sharedBudget.collaborators.find(c => isEmailMatch(c.email, cleanEmail));
                      if (collab) {
                        collab.accessMode = expectedAccessMode;
                      }
                    }
                  }
                }
      ...
      this.isSyncingSharedBudgets = false;
    }
    return this.deduplicateSharedBudgets();
  }
  ...
  static addNotification(notif: Omit<BudgetNotification, 'id' | 'createdAt' | 'status'>): BudgetNotification {
    ...
    // Appwrite Notificacoes collection persistence
    try {
      const cfg = getAppwriteConfig();
      if (cfg.databaseId) {
        // Ensure consistent casing: 'Notificacoes'
        appwriteDatabases.createDocument(cfg.databaseId, 'Notificacoes', ID.unique(), {
          userId: notif.toEmail.toLowerCase().trim(),
          budgetId: notif.budgetId || '',
          mensagem: notif.message,
          tipo: notif.type
        }).catch(e => console.warn('[Appwrite Notification Error]', e));
      }
    } catch (err) {}
  ...
  static respondToNotification(notifId: string, action: 'accept' | 'reject', currentUser: User): { success: boolean; message: string; updatedUser?: User } {
    ...
    if (isAccepted) {
      ...
      if (targetBudget) {
        ...
        // Also update that person's user profile in USERS (keeping personal budgetId)
        const usersStr = localStorage.getItem(STORAGE_KEYS.USERS) || '[]';
        const users: User[] = JSON.parse(usersStr);
        ...
        if (currentUser.email.toLowerCase() === personToAddEmail.toLowerCase()) {
          localStorage.setItem('dsf_current_active_budget', targetBudget.budgetId);
          localStorage.setItem('darla_active_budget_id', targetBudget.budgetId);
          localStorage.setItem('dsf_viewing_shared_budget', 'true');
        }
  ...
  static async joinBudgetByCodeOrEmail(
    ...
    if (targetBudget.collaborators.some((c) => isEmailMatch(c.email, currentUser.email))) {
      // Re-verify against Appwrite to see if access was actually revoked
      try {
        const cfg = getAppwriteConfig();
        const dbId = cfg.databaseId;
        if (dbId) {
          const ownerEmail = targetBudget.ownerEmail.toLowerCase().trim();
          const ownerDocId = getCanonicalAppwriteDocId(ownerEmail);
          let ownerDoc: any = null;
          try {
            ownerDoc = await appwriteDatabases.getDocument(dbId, 'user_financials', ownerDocId);
          } catch (e) {
            const listRes = await appwriteDatabases.listDocuments(dbId, 'user_financials', [Query.equal('userId', ownerEmail)]);
            if (listRes.documents.length > 0) ownerDoc = listRes.documents[0];
          }

          if (ownerDoc) {
            const ownerData = typeof ownerDoc.data === 'string' ? JSON.parse(ownerDoc.data || '{}') : (ownerDoc.data || {});
            const allowed = Array.isArray(ownerData.allowed_users) ? ownerData.allowed_users : (Array.isArray(ownerData.shared_members) ? ownerData.shared_members : []);
            const myEmail = currentUser.email.toLowerCase().trim();
            const isStillAllowed = allowed.some((m: string) => m && m.toLowerCase().trim() === myEmail);

            if (!isStillAllowed) {
              console.log('[DEBUG] Acesso foi revogado anteriormente. Permitindo nova solicitação.');
              // Limpa cache local para este orçamento
              targetBudget.collaborators = targetBudget.collaborators.filter(c => !isEmailMatch(c.email, currentUser.email));
              localStorage.setItem(STORAGE_KEYS.SHARED_BUDGETS, JSON.stringify(allBudgets));
            } else {
        ...
        console.warn('[DEBUG] Falha ao verificar acesso em tempo real, usando cache.', e);
        return { success: false, message: 'Você já possui acesso a este orçamento.' };
      }
    }
    ...
    // Force creation of a NEW request if we want to ensure persistence, or just update the existing one properly.
    // Instead of re-sending, let's always update the Appwrite document to ensure the titular sees it.
    console.log('[DEBUG] Processing budget request. Existing:', !!existingPending);

    // 1. In-app notification for budget owner (Auto-syncs to Appwrite Notificacoes collection)
    this.addNotification({
      type: 'request',
      fromUserId: currentUser.id,
      fromName: currentUser.name,
    ...
    // 2. BACKWARD COMPATIBILITY: Update owner's user_financials document directly
    try {
      const cfg = getAppwriteConfig();
      const dbId = cfg.databaseId;
      if (dbId) {
        ...
        try {
          ...
          // If not found by ID, try listing by userId
          const listRes = await appwriteDatabases.listDocuments(dbId, 'user_financials', [Query.equal('userId', targetEmail)]);
          if (listRes.documents.length > 0) ownerDoc = listRes.documents[0];
        }

        if (ownerDoc) {
          const ownerData = typeof ownerDoc.data === 'string' ? JSON.parse(ownerDoc.data || '{}') : (ownerDoc.data || {});
          if (!Array.isArray(ownerData.pedidos_acesso)) ownerData.pedidos_acesso = [];
          
          // Deduplicate
          ownerData.pedidos_acesso = ownerData.pedidos_acesso.filter((p: any) => !isEmailMatch(p.from_email, currentUser.email));
          
          ownerData.pedidos_acesso.push({
            id: `req_${Date.now()}`,
          ...
          await appwriteDatabases.updateDocument(dbId, 'user_financials', ownerDoc.$id, {
            data: JSON.stringify(ownerData)
          });
          console.log('[DEBUG] Dual persistence: user_financials updated successfully.');
        }
      }
    } catch (err) {
      console.warn('[DEBUG] Dual persistence failed (non-critical):', err);
    }
  ...
  static switchBudget(currentUser: User, newBudgetId?: string): User {
    this.initialize();
    const userKey = (currentUser.email || currentUser.id || 'default').toLowerCase();
    const personalBudgetId = getCanonicalUserId(currentUser.email || currentUser.id || 'default');
    const isPersonal = !newBudgetId || 
      newBudgetId === personalBudgetId || 
      newBudgetId === currentUser.id || 
      newBudgetId === currentUser.email || 
      getCanonicalUserId(newBudgetId) === personalBudgetId;

    const targetBudgetId = isPersonal ? personalBudgetId : newBudgetId;
    const isShared = !isPersonal;

    localStorage.setItem(`dsf_active_budget_${userKey}`, targetBudgetId);
    localStorage.setItem(`dsf_viewing_shared_budget_${userKey}`, isShared ? 'true' : 'false');

    const updatedUser: User = {
      ...currentUser,
      budgetId: personalBudgetId,
      activeBudgetId: targetBudgetId,
      isViewingSharedBudget: isShared,
      lastBudgetSwitch: Date.now(),
    } as any;
  ...
  static async updateCollaboratorAccessMode(
    budgetId: string,
    emailToUpdate: string,
    accessMode: 'edit' | 'read'
  ): Promise<SharedBudget | null> {
    this.initialize();
    console.log("🕵️ [DEDO-DURO] updateCollaboratorAccessMode called:", { budgetId, emailToUpdate, accessMode });
    const allStr = localStorage.getItem(STORAGE_KEYS.SHARED_BUDGETS) || '[]';
    const allBudgets: SharedBudget[] = JSON.parse(allStr);
  ...
  static isBudgetOwner(user: User, budgetId?: string): boolean {
    if (!user) return false;
    this.initialize();

    const userKey = (user.email || user.id || 'default').toLowerCase();
    const personalBudgetId = getCanonicalUserId(user.email || user.id || 'default');
    const targetBudgetId = budgetId || this.getEffectiveBudgetId(user);

    const isViewingShared = localStorage.getItem(`dsf_viewing_shared_budget_${userKey}`) === 'true';

    // Se estiver no orçamento pessoal direto
    if (
      !isViewingShared ||
      targetBudgetId === personalBudgetId ||
      targetBudgetId === user.id ||
      targetBudgetId === user.email ||
      getCanonicalUserId(targetBudgetId) === personalBudgetId
    ) {
      return true;
    }

    // Se estiver visualizando outro orçamento, checa se é o titular/dono registrado daquele orçamento
    const allBudgets = this.deduplicateSharedBudgets();
    const budget = allBudgets.find(b => 
      b.budgetId === targetBudgetId || 
      getCanonicalUserId(b.budgetId) === targetBudgetId ||
      b.ownerEmail?.toLowerCase() === targetBudgetId.toLowerCase() ||
      b.ownerId === targetBudgetId
    );

    if (budget) {
      const myEmail = (user.email || '').trim().toLowerCase();
      const myId = String(user.id || '').trim().toLowerCase();
      const myCanonicalId = getCanonicalUserId(user.id || user.email || '');
      const ownerEmail = (budget.ownerEmail || '').trim().toLowerCase();
      const ownerId = String(budget.ownerId || '').trim().toLowerCase();

      if (
        (myEmail && (ownerEmail === myEmail || isEmailMatch(ownerEmail, myEmail))) ||
        (myId && (ownerId === myId || ownerId === myCanonicalId))
      ) {
        return true;
      }
      return false;
    ...
    return targetBudgetId === personalBudgetId;
  }
  ...
  static isCurrentUserReadOnly(user: User): boolean {
    ...
    const meuEmailLido = String(user?.email || '').toLowerCase().trim();
    ...
    // 1. Check member_permissions from user financials in localStorage FIRST (Authoritative owner setting)
    try {
      const allFinStr = localStorage.getItem('DINHEIRO_SEM_FILTRO_USER_FINANCIALS') || '{}';
      const allFin = JSON.parse(allFinStr);
      for (const k of Object.keys(allFin)) {
        ...
        const ownerCandidate = String(item.email || parsedData.owner_email || parsedData.titular || k).toLowerCase().trim();
        if (ownerCandidate === emailDoDono || item.id === effectiveBudgetId || k === effectiveBudgetId || k.toLowerCase() === emailDoDono || ownerCandidate.includes(emailDoDono) || emailDoDono.includes(ownerCandidate)) {
          ...
          for (const permKey of Object.keys(perms)) {
            if (permKey.toLowerCase().trim() === meuEmailLido) {
              ...
              if (val === 'leitura' || val === 'read') return true;
              if (val === 'edicao' || val === 'edit' || val === 'edição') return false;
            }
          }
          if (perms[meuEmailLido]) {
            const val = String(perms[meuEmailLido]).toLowerCase().trim();
            if (val === 'leitura' || val === 'read') return true;
            if (val === 'edicao' || val === 'edit' || val === 'edição') return false;
          }
        }
    ...
    let collab = budget.collaborators.find((c) => String(c.email || '').toLowerCase().trim() === meuEmailLido);
    if (!collab) {
      const allBudgets = this.deduplicateSharedBudgets();
      const targetB = allBudgets.find(b => b.budgetId === effectiveBudgetId || String(b.ownerEmail || '').toLowerCase().trim() === emailDoDono);
      if (targetB && Array.isArray(targetB.collaborators)) {
        collab = targetB.collaborators.find(c => String(c.email || '').toLowerCase().trim() === meuEmailLido);
      }
    }
    ...
    if (collab && collab.accessMode) {
      ...
      if (mode === 'edit' || mode === 'edicao' || mode === 'edição') return false;
    }

    return false;
  }

  static getUserAccessModeForBudget(user: User, ownerEmail: string): 'read' | 'edit' {
    this.initialize();
    if (!ownerEmail) return 'edit';
    const meuEmailLido = String(user?.email || '').toLowerCase().trim();
    ...
    try {
      ...
      for (const k of Object.keys(allFin)) {
        ...
        const parsedData = item.data ? (typeof item.data === 'string' ? JSON.parse(item.data) : item.data) : {};
        const ownerCandidate = String(item.email || parsedData.owner_email || parsedData.titular || k).toLowerCase().trim();
        if (ownerCandidate === ownerNormalized || k.toLowerCase() === ownerNormalized) {
          const perms = parsedData.member_permissions || {};
          for (const permKey of Object.keys(perms)) {
            if (permKey.toLowerCase().trim() === meuEmailLido) {
              const val = String(perms[permKey]).toLowerCase().trim();
    ...
    if (user && user.email && user.email.toLowerCase() !== ownerNormalized) {
      try {
        if (typeof window !== 'undefined' && !(window as any).__dedoDuroAlertShown) {
          ...
          console.log('[DEDO-DURO LEITURA] Permissão lida no banco para este orçamento: ' + modoLido + ' | Documento ID: ' + ownerNormalized + ' (Origem: ' + origem + ')');
        }
      } catch(e) {}
    }
  ...
  static getCategories(userId: string): Category[] {
    ...
    const deletedIds = this.getDeletedIds(canonicalId);
    const key = `darla_categories_${canonicalId}`;
    try {
      const raw = localStorage.getItem(key);
      if (raw) {
        const parsed = JSON.parse(raw);
        if (Array.isArray(parsed) && parsed.length >= 0) {
          const valid = parsed.filter((c: any) => c && c.id && !deletedIds.has(c.id)).map((c: any) => ({ ...c, userId: canonicalId }));
          if (valid.length > 0) return valid;
        }
      }
    } catch (e) {}

    let userCats = _inMemoryStore.categories.filter((c) => getCanonicalUserId(c.userId) === canonicalId && !deletedIds.has(c.id));
    if (userCats.length === 0) {
      userCats = SEED_CATEGORIES.map((c) => ({ ...c, id: `${c.id}_${canonicalId}`, userId: canonicalId })).filter(c => !deletedIds.has(c.id));
    }
    this.setCategories(userCats, canonicalId);
    return userCats;
  }

  static saveCategory(category: Category, budgetId?: string): Category {
    const currUser = this.getCurrentUser();
    const effectiveBudget = budgetId 
      ? resolveBudgetId(budgetId) 
      : (category.userId ? resolveBudgetId(category.userId) : (currUser ? this.getEffectiveBudgetId(currUser) : 'default'));
    const canonicalId = getCanonicalUserId(effectiveBudget);
    const catToSave = { ...category, userId: canonicalId, updatedAt: new Date().toISOString(), _pendingSync: true };
    const cats = this.getCategories(canonicalId);
    
    const normalizeCatId = (id: string) => String(id || '').replace(/_[^_@]+@[^.]+.*$/, '');
    const isSameCat = (aId: string, bId: string) => aId === bId || normalizeCatId(aId) === normalizeCatId(bId);

    const index = cats.findIndex((c) => isSameCat(c.id, catToSave.id));
    if (index >= 0) {
      cats[index] = {
        ...cats[index],
        ...catToSave,
        subcategories: catToSave.subcategories || cats[index].subcategories || [],
      };
    } else {
      cats.push(catToSave);
    }
    this.setCategories(cats, canonicalId);
    recordPendingCategoryMutation(catToSave.id, 'updateCategory', catToSave);
    pushCategoryToFirestore(catToSave);
    this.syncUserMutationToServer(canonicalId);
    return catToSave;
  }

  static deleteCategory(categoryId: string, budgetId?: string) {
    const cat = _inMemoryStore.categories.find((c) => c.id === categoryId);
    const currUser = this.getCurrentUser();
    const effectiveBudget = budgetId 
      ? resolveBudgetId(budgetId) 
      : (cat?.userId ? resolveBudgetId(cat.userId) : (currUser ? this.getEffectiveBudgetId(currUser) : 'default'));
    const canonicalId = getCanonicalUserId(effectiveBudget);

    const cats = this.getCategories(canonicalId).filter((c) => c.id !== categoryId);
    this.setCategories(cats, canonicalId);
  ...
  static saveAccount(account: Account, budgetId?: string): Account {
    ...
    const effectiveBudget = budgetId 
      ? resolveBudgetId(budgetId) 
      : (account.userId ? resolveBudgetId(account.userId) : (currUser ? this.getEffectiveBudgetId(currUser) : 'default'));
    const canonicalId = getCanonicalUserId(effectiveBudget);
    ...
    const accounts = this.getAccounts(canonicalId);
    const index = accounts.findIndex((a) => a.id === accToSave.id);
    if (index >= 0) {
      accounts[index] = { ...accounts[index], ...accToSave };
    } else {
  ...
  static setCategories(categories: Category[], budgetId?: string) {
    this.initialize();
    const canonicalId = getCanonicalUserId(budgetId || categories[0]?.userId || resolveBudgetId());
    const key = `darla_categories_${canonicalId}`;
    const updated = categories.map((c) => ({ ...c, userId: canonicalId }));
    _inMemoryStore.categories = _inMemoryStore.categories.filter((c) => getCanonicalUserId(c.userId) !== canonicalId).concat(updated);
    try {
  ...
  static setFamilyMembers(familyMembers: FamilyMember[], budgetId?: string) {
    this.initialize();
  ...
  static setTransactions(transactions: Transaction[], budgetId?: string) {
    this.initialize();
  ...
  static setGoals(goals: Goal[], budgetId?: string) {
    this.initialize();
  ...
  // --- TRANSACTIONS ---
  static getTransactions(budgetId: string): Transaction[] {
    this.initialize();
    const canonicalId = getCanonicalUserId(budgetId || 'default');
    this.purgeDeletedItems(canonicalId);
    const deletedIds = this.getDeletedIds(canonicalId);
  ...
  static addTransaction(transaction: Omit<Transaction, 'id' | 'createdAt'>): Transaction {
    const canonicalId = resolveBudgetId(transaction.userId);
    ...
    const txs = this.getTransactions(canonicalId);
    txs.push(newTx);
  ...
  static addMultipleTransactions(transactions: Omit<Transaction, 'id' | 'createdAt'>[]): Transaction[] {
    ...
    Object.entries(addedByBudget).forEach(([canonicalId, txs]) => {
      this.setTransactions(txs, canonicalId);
      this.syncUserMutationToServer(canonicalId);
    });
  ...
  static toggleConsolidated(transactionId: string): Transaction | null {
    ...
    if (tx) {
      ...
      const canonicalId = getCanonicalUserId(tx.userId);
      const txs = this.getTransactions(canonicalId);
  ...
  static getGoals(userId: string): Goal[] {
    ...
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw);
        if (Array.isArray(parsed) && parsed.length > 0) {
          const valid = parsed.filter((g: any) => g && g.id && !deletedIds.has(g.id)).map((g: any) => ({ ...g, userId: canonicalId }));
          if (valid.length > 0) return valid;
        }
      }
    } catch (e) {}

    return _inMemoryStore.goals.filter((g) => getCanonicalUserId(g.userId) === canonicalId && !deletedIds.has(g.id));
  }

  static saveGoal(goal: Goal): Goal {
    const canonicalId = resolveBudgetId(goal.userId);
    const updatedGoal = { ...goal, userId: canonicalId, updatedAt: new Date().toISOString(), _pendingSync: true };
    const goals = this.getGoals(canonicalId);
    ...
    this.setGoals(goals, canonicalId);
  ...
  static updateGoalProgress(goalId: string, addedAmount: number): Goal | null {
    ...
    if (goal) {
      ...
      this.setGoals(this.getGoals(canonicalId), canonicalId);
  ...
  static deleteGoal(goalId: string, budgetId?: string) {
    const goal = _inMemoryStore.goals.find((g) => g.id === goalId);
    const currUser = this.getCurrentUser();
    const targetUserId = budgetId || goal?.userId || currUser?.id || '';
    const canonicalId = resolveBudgetId(targetUserId);

    const goals = this.getGoals(canonicalId).filter((g) => g.id !== goalId);
    this.setGoals(goals, canonicalId);
  ...
  static getFamilyMembers(userId: string): FamilyMember[] {
    ...
    this.setFamilyMembers(fam, canonicalId);
    return fam;
  }

  static saveFamilyMember(member: FamilyMember): FamilyMember {
    const canonicalId = resolveBudgetId(member.userId);
    const updatedMember = { ...member, userId: canonicalId, updatedAt: new Date().toISOString(), _pendingSync: true };
    const members = this.getFamilyMembers(canonicalId);
    const index = members.findIndex((f) => f.id === member.id);
    if (index >= 0) {
      members[index] = updatedMember;
    } else {
      members.push(updatedMember);
    }
    this.setFamilyMembers(members, canonicalId);
    pushFamilyMemberToFirestore(updatedMember);
    this.syncUserMutationToServer(canonicalId);
    return updatedMember;
  }

  static deleteFamilyMember(id: string, budgetId?: string) {
    const fm = _inMemoryStore.familyMembers.find((f) => f.id === id);
    const currUser = this.getCurrentUser();
    const targetUserId = budgetId || fm?.userId || currUser?.id || '';
    const canonicalId = resolveBudgetId(targetUserId);

    const members = this.getFamilyMembers(canonicalId).filter((f) => f.id !== id);
    this.setFamilyMembers(members, canonicalId);
  ...
  static async deleteUserAccount(userId: string, requestingUser?: User): Promise<void> {
    this.initialize();
    ...
    // Validação estrita de titularidade da conta
    const u = requestingUser || this.getCurrentUser();
    if (u) {
      const canonicalReq = getCanonicalUserId(u.id || u.email || '');
      const canonicalTarget = getCanonicalUserId(userId);
      const isOwner = (canonicalReq === canonicalTarget) || (u.id === userId) || (u.email && u.email.toLowerCase() === cleanInput);
      if (!isOwner) {
        console.warn('[SEGURANÇA] Bloqueio: Usuário não é o titular da conta a ser excluída', { user: u.email, target: userId });
        throw new Error('Ação bloqueada: Apenas o titular da conta possui permissão para excluí-la.');
      }
    }
    ...
    const userEmail = isInputEmail ? cleanInput : (targetUser?.email ? targetUser.email.toLowerCase() : (userId.includes('@') ? userId.trim().toLowerCase() : (u?.email ? u.email.toLowerCase() : '')));
    ...
    try {
      await fetch('/api/users/delete', {
        ...
        body: JSON.stringify({ 
          userId, 
          email: userEmail,
          requestingUserId: u ? (u.id || u.email) : userId 
        }),
      });
    } catch (e) {
    ...
    // 9. Async cleanup in Firebase Firestore & Appwrite
    ...
    try {
      ...
      if (config.projectId && config.projectId !== 'default-placeholder') {
        ...
        const idsToDelete = [
          userEmail ? getCanonicalAppwriteDocId(userEmail) : '',
          userId ? getCanonicalAppwriteDocId(userId) : '',
          canonicalId ? getCanonicalAppwriteDocId(canonicalId) : '',
          userEmail ? `user_${userEmail.replace(/[^a-zA-Z0-9]/g, '_')}` : '',
          userEmail ? `usr_${userEmail.replace(/[^a-zA-Z0-9]/g, '_')}` : '',
          userId ? `user_${userId.replace(/[^a-zA-Z0-9]/g, '_')}` : ''
        ].filter(Boolean);

        for (const id of idsToDelete) {
          try {
            await appwriteDatabases.deleteDocument(config.databaseId, 'user_financials', id);
          } catch (e) {}
        }
        ...
        for (const doc of list.documents) {
          try {
            const docUserId = String(doc.userId || '').trim().toLowerCase();
            const docId = String(doc.$id || '').trim().toLowerCase();
            const matchesUser = 
              (userEmail && (docUserId === userEmail || docId.includes(userEmail) || docUserId.includes(userEmail))) ||
              (userId && (docUserId === userId.toLowerCase() || docId === userId.toLowerCase())) ||
              (canonicalId && (docUserId === canonicalId || docId === canonicalId));

            if (matchesUser) {
              console.log('[Appwrite Delete] Excluindo documento de usuário em user_financials:', doc.$id);
              ...
              if (modified) {
                await appwriteDatabases.updateDocument(config.databaseId, 'user_financials', doc.$id, {
            ...
            console.warn('[Appwrite Delete Doc Error]', e);
          }
        }
      }
    } catch (e) {
      console.warn('[deleteUserAccount Appwrite cleanup error]', e);
    }
  ...
  static clearPendingSync(budgetId?: string) {
    if (typeof window === 'undefined') return;
    try {
      const u = this.getCurrentUser();
      const targetBudgetId = budgetId || (u ? this.getEffectiveBudgetId(u) : 'default');
      
      const txsKey = STORAGE_KEYS.TRANSACTIONS + '_' + targetBudgetId;
      const txs = this.getTransactions(targetBudgetId).map((t: any) => ({ ...t, _pendingSync: false }));
      localStorage.setItem(txsKey, JSON.stringify(txs));
      localStorage.setItem(STORAGE_KEYS.TRANSACTIONS, JSON.stringify(txs));
      
      const accsKey = STORAGE_KEYS.ACCOUNTS + '_' + targetBudgetId;
      const accs = this.getAccounts(targetBudgetId).map((a: any) => ({ ...a, _pendingSync: false }));
      localStorage.setItem(accsKey, JSON.stringify(accs));
      localStorage.setItem(STORAGE_KEYS.ACCOUNTS, JSON.stringify(accs));

      const catsKey = STORAGE_KEYS.CATEGORIES + '_' + targetBudgetId;
      const cats = this.getCategories(targetBudgetId).map((c: any) => ({ ...c, _pendingSync: false }));
      localStorage.setItem(catsKey, JSON.stringify(cats));
      localStorage.setItem(STORAGE_KEYS.CATEGORIES, JSON.stringify(cats));

      const goalsKey = STORAGE_KEYS.GOALS + '_' + targetBudgetId;
      const goals = this.getGoals(targetBudgetId).map((g: any) => ({ ...g, _pendingSync: false }));
      localStorage.setItem(goalsKey, JSON.stringify(goals));
      localStorage.setItem(STORAGE_KEYS.GOALS, JSON.stringify(goals));

      const famKey = STORAGE_KEYS.FAMILY_MEMBERS + '_' + targetBudgetId;
      const fam = this.getFamilyMembers(targetBudgetId).map((f: any) => ({ ...f, _pendingSync: false }));
      localStorage.setItem(famKey, JSON.stringify(fam));
      localStorage.setItem(STORAGE_KEYS.FAMILY_MEMBERS, JSON.stringify(fam));
