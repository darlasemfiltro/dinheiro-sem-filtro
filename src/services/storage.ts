...
import { getAppwriteUser } from '../lib/appwrite';
...
export class StorageService {
    ...
    try {
      ...
      if (appwriteConnData) {
        ...
        if (appwriteConnData.user) {
          _inMemoryStore.currentUser = { ..._inMemoryStore.currentUser, ...appwriteConnData.user, id: canonicalId };
        }
      }
  ...
  static logout(_clearUserData = false, _targetUserId?: string) {
    try {
      if (typeof window !== 'undefined') {
        ...
        // Financial records are safely preserved in local cache and server cloud sync
      }
    } catch (e) {
  ...
  static getSharedBudget(budgetId: string, ownerUser?: User): SharedBudget {
    ...
    const cleanEmail = ownerUser ? (ownerUser.email || '').trim().toLowerCase() : '';
    const canonicalId = cleanEmail ? getCanonicalUserId(cleanEmail) : getCanonicalUserId(budgetId);

    let budget = allBudgets.find((b) => b.budgetId === canonicalId || b.budgetId === budgetId);
    ...
    if (!budget) {
      const code = ownerUser?.sharedBudgetCode || (cleanEmail ? getDeterministicBudgetCode(cleanEmail, ownerUser?.name) : 'ORCAMENTO-1000');

      budget = {
        budgetId: canonicalId,
        ownerId: canonicalId,
        ownerName: ownerUser ? ownerUser.name : (cleanEmail ? cleanEmail.split('@')[0] : 'Titular do Orçamento'),
        ownerEmail: cleanEmail || (ownerUser ? ownerUser.email : 'titular@exemplo.com'),
        code,
        collaborators: [],
      };
      ...
      // Update ownerName if user changed name
      if (ownerUser && ownerUser.name && ownerUser.id === budget.ownerId && budget.ownerName !== ownerUser.name) {
        budget.ownerName = ownerUser.name;
        updated = true;
      }

      // Update ownerEmail if ownerUser email is available and differs or was placeholder
      if (ownerUser && ownerUser.email && (budget.ownerEmail !== ownerUser.email || budget.ownerEmail === 'titular@exemplo.com')) {
      ...
      if (cleanEmail) {
        const canonicalCode = ownerUser?.sharedBudgetCode || getDeterministicBudgetCode(cleanEmail, ownerUser?.name || budget.ownerName);
  ...
  static async addCollaboratorByEmail(
    ...
    // 2. Check server database lookup if not found locally
    if (!guestUserExists) {
      try {
        const res = await fetch(`/api/users/lookup?email=${encodeURIComponent(cleanEmail)}`);
        if (res.ok) {
