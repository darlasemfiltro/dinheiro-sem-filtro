...
export class GamificationService {
  static getGamificationState(userId: string): WeeklyGamificationState {
    ...
    // Automatically update and complete weekly check-in upon login / app load (skip for brand new accounts)
    const isBrandNew = state.xpTotal === 0 && state.weeklyStreakCount === 0 && (!state.completedWeeksHistory || state.completedWeeksHistory.length === 0);

    if (!isBrandNew && !state.hasCompletedWeeklyCheckIn) {
      state.hasCompletedWeeklyCheckIn = true;
      state.xpTotal = (Number(state.xpTotal) || 0) + 60;
      state.weeklyXP = (Number(state.weeklyXP) || 0) + 60;
      state.gems = (Number(state.gems) || 0) + 25;

      if (!state.completedWeeksHistory) {
        state.completedWeeksHistory = [];
      }
      if (!state.completedWeeksHistory.includes(currentWeekKey)) {
        state.completedWeeksHistory.push(currentWeekKey);
        state.weeklyStreakCount = (state.weeklyStreakCount || 0) + 1;
      }
      ...
      setTimeout(() => {
        GamificationService.syncGamificationWithServer(state).catch(() => {});
      }, 500);
    }
